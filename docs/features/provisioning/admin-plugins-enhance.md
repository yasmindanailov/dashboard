...
| 13 | Trigger manual del cron L3. **Endpoint canónico `POST /api/v1/admin/plugins/enhance_cp/reconcile-all`** (Sprint 15C.II Fase B — [ADR-083 Amendment A4 §A4.2](../../10-decisions/adr-083-plugin-enhance-cp-specifics.md#amendment-a4-2026-05-10--hardening-ux-post-smoke-real-yasmin-sprint-15cii)). Pre-Fase B: workaround único = esperar 6h o invocar `cron.runOnce()` vía REPL/script de test. ⚠ Iteraciones previas de esta doc afirmaban `POST /api/v1/admin/cron/enhance-reconciliation` — endpoint **nunca existió** (vaporware corregido 2026-05-10). | Si NO hay drift: log "no drift detected". Si drift forzado (ej. cambiar plan en Enhance UI sin pasar por Aelium): emite `service.reconciled_external_change` con `change_type='plan_divergence'`. Reusa el mismo pipeline `reconcileService(serviceId)` que el cron @Cron(EVERY_6_HOURS) ejecuta automáticamente. |
