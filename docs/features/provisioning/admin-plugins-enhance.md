...
| 13 | Trigger manual del cron L3 vía endpoint canónico **`POST /api/v1/admin/plugins/enhance_cp/reconcile-all`** ✅ implementado Sprint 15C.II Fase B ([PR #54](https://github.com/yasmindanailov/dashboard/pull/54)) + UI admin: botón "↻ Reconciliar todos ahora" en `/admin/settings/plugins/enhance-cp`. Materializa [ADR-083 Amendment A4.2](../../10-decisions/adr-083-plugin-enhance-cp-specifics.md#amendment-a4-2026-05-10--hardening-ux-post-smoke-real-yasmin-sprint-15cii). Doble propósito: A2 (UX general) + G1 (smoke testing manual sin esperar 6h). | HTTP 201 + JSON `{slug, triggered_at, services_processed, drifts_detected, duration_ms, details}`. Audit canónico `plugin.reconcile_triggered_manually`. Emite `service.reconciled_external_change` por cada drift detectado con `change_type ∈ {subscription_missing, status_divergence, plan_divergence}`. Mismo pipeline que el cron `@Cron(EVERY_6_HOURS)` automático. |
