...
### DH-INV-N (Domain↔Hosting invariants)
Conjunto de 6 invariantes canónicas declaradas en [ADR-082 §1](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md) (Sprint 15C Fase A) que el modelo Domain↔Hosting debe respetar transversalmente — todos los plugins hosting/registrar futuros las cumplen. **DH-INV-1**: hosting service SIEMPRE tiene FQDN (`service.domain` no nulo). **DH-INV-2**: hosting plugin rechaza `provision()` si `service.domain` null o malformed (`INVALID_PAYLOAD`). **DH-INV-3**: domain service puede vivir solo, sin hosting asociado obligatorio. **DH-INV-4**: linkage Domain ↔ Hosting es string `services.domain`, NO foreign key (permite "bring your own domain" externo). **DH-INV-5**: renewal cycles independientes — cancelar uno NO cancela el otro. **DH-INV-6 ⭐**: en conflicto operacional, el panel del proveedor (Enhance) gana sobre Aelium — Aelium NO es fuente de verdad operacional, es gateway curado de billing + identidad.

### NS-sync 3 capas (C1/C2/C3)
Patrón canónico ([ADR-082 §4](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md)) para mantener coherencia de los nameservers `ns1/ns2.aelium.net` en los 3 lugares donde deben coincidir: **C1** glue records de `aelium.net` en Cloudflare zone + WHOIS del registrar de `aelium.net` (manual ops, NO automático); **C2** default NS de zonas Enhance vía `POST /v2/settings/dns/default-records` (bootstrap automático del plugin); **C3** setting Aelium `provisioning.default_nameservers` (fuente de verdad — listener `provisioning.default_nameservers_changed` propaga a C2 vía API). Cambio en C3 → propagación automática a C2; C1 sigue manual porque vive fuera del cluster Enhance.

### DNS authority resolver (`core/provisioning/dns-authority-resolver.ts`)
Helper canónico cross-plugin ([ADR-082 §6](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md)) que el orquestador usa para responder *"¿quién es la autoridad DNS de este service?"* sin que un plugin importe a otro (R4). Compara `service.metadata.nameservers` vs `Setting.provisioning.default_nameservers` (cuando `service.product_type === 'domain'`) o devuelve directamente el plugin Enhance (cuando `product_type === 'hosting_*'`). Resultado: `{ authority: 'aelium' | 'external', plugin: ProvisionerPlugin | null }`. Consumido por endpoint `GET/POST/PATCH/DELETE /api/v1/services/{id}/dns/records` (Fase 15C.D) que routea al plugin con `has_dns_management=true`. Vive en `core/provisioning/`, NO en plugin individual — el plugin RC NO importa el plugin Enhance.

### Default DNS records platform-level
Mecanismo canónico ([ADR-082 §3.5](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md) + [ADR-083 §6.5](../10-decisions/adr-083-plugin-enhance-cp-specifics.md)) para configurar los records DNS que TODA zona nueva creada en el cluster Enhance hereda automáticamente. Endpoint Enhance `POST /v2/settings/dns/default-records`. Aelium configura una sola vez en bootstrap del plugin Enhance: `{kind:'A', name:'@', value:<server_ip>}`, `{kind:'A', name:'www', value:<server_ip>}`, `{kind:'NS', name:'@', value:'ns1.aelium.net'}`, `{kind:'NS', name:'@', value:'ns2.aelium.net'}`. Cada zona creada después hereda esos records sin código en runtime de provision. **Reemplaza** el listener inline `auto-config-dns-on-hosting-provisioned` que el dossier 15D pre-fijó — ese listener queda como reconciliation defensivo (verifica que la zona tiene los defaults; los re-aplica si faltan).

### Checkout flows F1/F2/F3/F4
Cuatro flujos canónicos de checkout de un cliente que combinan dominio + hosting ([ADR-082 §3.2](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md)). **F1** Register new domain + buy hosting (60-70% industria) — 2 line items en misma factura, registrar primero (síncrono RC), hosting después (Enhance), default DNS records globales se aplican a la zona auto-creada, 2 services con renewal cycles independientes desde día 1. **F2** Use existing Aelium-managed domain + buy hosting — 1 line item, hosting service se crea con `domain=<FQDN existente>`. **F3** BYOD (Bring Your Own Domain externo) + buy hosting — 1 line item, NO existe service Aelium para ese dominio, Aelium presenta instrucciones para configurar A records externos o cambiar NS a Aelium. **F4** Transfer-in domain + buy hosting — 2 line items, hosting se provisiona inmediatamente con dominio externo (estado F3 transitorio), transfer-in arranca asíncrono (5-7 días), evento `domain.transfer_completed` cierra el flow.

### Enhance Customer
Mapping canónico ([ADR-083 §2 decisión 7](../10-decisions/adr-083-plugin-enhance-cp-specifics.md) + [Amendment A2](../10-decisions/adr-083-plugin-enhance-cp-specifics.md#amendments)) entre Client Aelium y la sub-org Enhance que lo representa. Persistido en tabla `enhance_customers` con PK natural `user_id` (FK a `users.id`, naming SQL alineado con la convención del schema — la doctrina conceptual sigue siendo "Client Aelium ↔ Customer Org Enhance"). Lazy create al primer hosting Enhance del cliente, NO en alta del User. Cardinalidad: 1 fila por User Aelium con al menos un hosting Enhance contratado. Schema canónico: [enhance-customers.md](../30-data/enhance-customers.md).

### Master Org Aelium
La organización raíz en Enhance (UUID configurado en `manifest.configSchema.masterOrgId` del plugin `enhance_cp`) bajo la que cuelgan TODOS los Customer Org Enhance del Aelium ([ADR-083 §1 decisión 4](../10-decisions/adr-083-plugin-enhance-cp-specifics.md)). Aelium opera como Master Org → cada cliente Aelium con hosting es un sub-customer. El bearer token Super Admin tiene permisos sobre el Master Org y todos sus sub-customers. NO existe sub-resellers v1 (un cliente Aelium NO puede ser Master de sus propios sub-customers — DC.NEW-15C-RESELLER en backlog).

### Customer Org Enhance
La sub-org Enhance que representa a un Client Aelium concreto dentro del Master Org Aelium ([ADR-083 §2 decisión 7](../10-decisions/adr-083-plugin-enhance-cp-specifics.md)). Owner-controlled: tiene exactamente un Owner (rol no borrable en Enhance) que es el login del cliente. Identificadores cacheados en `enhance_customers`: `enhance_org_id` (UUID), `enhance_owner_login_id`, `enhance_owner_member_id`. Aloja N subscriptions y N websites — un mismo Customer Org puede tener varios hostings contratados.

### OTP SSO URL
URL temporal cred-less devuelta por `GET /orgs/{cust}/members/{m}/sso` ([ADR-083 §4 decisiones 13-15](../10-decisions/adr-083-plugin-enhance-cp-specifics.md)). Single-use + corto TTL (gestionado por Enhance, NO cacheado por Aelium). Forma `https://<panel>/login/sessions/sso?otp=<uuid>`. El navegador cliente la sigue con redirect 302; Enhance verifica el OTP, crea sesión cookie scopada al Customer Org y entra. El flujo cliente "Abrir mi panel" es 1 sola call API gracias a `enhance_owner_member_id` cacheado en `enhance_customers` (sin ese cache requeriría 2 calls: `GET /orgs/{cust}` para resolver `ownerId` + `GET /sso`). El flujo admin Aelium "Abrir panel cliente" emite además evento `service.admin_sso_impersonation` con flag GDPR `gdpr_visible_to_data_subject=true` que el portal RGPD `/dashboard/transparency` expone al cliente.

### Reconcile drift detection
Patrón canónico ([ADR-083 §6 decisiones 22-24](../10-decisions/adr-083-plugin-enhance-cp-specifics.md) + [DH-INV-6 ADR-082 §1](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md)) para detectar divergencias entre el estado en Aelium y el estado real en Enhance. Cron `reconcile-enhance-services` BullMQ cada 6h itera todos los `Service` con `provisioner_slug='enhance_cp'` + `status IN ('active','suspended')` y compara contra `GET /orgs/{org}/subscriptions/{sub_id}` + `GET /orgs/{org}/websites/{ws_id}`. Tres `change_type` canónicos detectables: `subscription_missing` (404 → marca status `'unknown'`, NO `'cancelled'` automático), `status_divergence` (Aelium adopta el estado de Enhance), `plan_divergence` (NO auto-corrige — billing implication, requiere decisión consciente del admin). Cada divergencia emite `service.reconciled_external_change`; si el threshold diario `provisioning.enhance_cp.reconciliation_alert_threshold` (default 5) se supera, el listener `notifications-on-reconciliation-threshold-exceeded` alerta al superadmin (operator pisando Aelium repetidamente = patrón patológico).

### Recovery Hint (`ServiceInfo.recoveryHint`)
Campo opcional del shape `ServiceInfo` que devuelve `getServiceInfo()` ([ADR-077 Amendment A5](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#amendments) — Sprint 15C.II Fase E). Cuando el plugin reporta un `status` de drift / proveedor inaccesible, `recoveryHint ∈ {'reprovision', 'reconcile', 'contact_support'}` indica la clase de remediación canónica que la UI debe ofrecer al admin. **El plugin es la única autoridad** sobre qué drift es recuperable y cómo — el frontend ramifica por este campo del contrato, NUNCA matchea `statusReason` por string (`statusReason` es i18n display, no contrato de comportamiento). `'reprovision'` → recurso ausente en el proveedor → `POST /admin/services/:id/reprovision`. `'reconcile'` → metadata local divergió (plan, refs) → re-sync del cron L3 manual (DH-INV-6: el proveedor manda). `'contact_support'` → drift no auto-remediable o proveedor caído → sin CTA accionable. Extensible: futuras clases se añaden a la unión + se documentan en el Amendment A5 + el frontend (`AdminDriftBanner`) las ramifica explícitamente. Heredable a todos los plugins SaaS — cada uno clasifica su drift al implementar `getServiceInfo()`.

### Recalculate provider metrics (`recalculate_provider_metrics`)
Inline action admin-only del plugin `enhance_cp` ([ADR-083 Amendment A5.1](../10-decisions/adr-083-plugin-enhance-cp-specifics.md#amendments) — Sprint 15C.II Fase E). **Renombrada desde `force_resync`** por naming honesto: pide al proveedor (Enhance) que recalcule activamente disco y ancho de banda de la subscription en su lado (`PUT /orgs/{org}/subscriptions/{sub}/calculate-resource-usage`) e invalida el cache `service_info` — **NO reconcilia nada** (eso es el cron L3) ni es el botón "↻ Refrescar" del `MetricsBar` (que solo re-lee la última métrica ya calculada). Caso de uso del ~5% (el proveedor no ha recalculado en mucho tiempo). Se opera desde `AdminServiceOperationsCard` (no la barra genérica "Acciones rápidas" — está en `INTERNAL_HELPER_SLUGS`), progressive disclosure con etiquetado preciso. El Amendment A4.2 anterior la llamaba inexactamente "Reconciliar contra Enhance" — A5.1 lo corrige.
