...
- ⏳ **Fase 15C.II.F** (próxima): admin overview operativo `/admin/settings/plugins/enhance-cp` (stats grid + tabla drifts + componente reusable) + suspend/unsuspend completo (capability `supports_suspend` ya frozen ADR-077 A4 — falta materializar: 2 inline actions adminOnly + wrappers `suspendServiceWithAudit`/`unsuspendServiceWithAudit` + endpoints + listeners email + UI) + GAP-15CII-M `/admin/services/[id]/audit` + `/dashboard/services/[id]/audit` timeline per-service + GAP-15CII-N `ErrorLogService.log()` con módulo origen en el wrapper + G4 cache TTL configurable + G5 breaker EnhanceApiClient + G8 bug test-connection.
- ⏳ **Fase 15C.II.G** (cierre): tests críticos faltantes (8 áreas) + E2E extension + retrospectiva en `completed/` + smoke final Yasmin contra mock + Enhance live.
