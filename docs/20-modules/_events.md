...
| `domain.transfer_initiated` / `domain.transfer_completed` / `domain.transfer_failed` | FSM de transfer ([ADR-084 §4](../10-decisions/adr-084-comercio-dominios-registrar.md) + [A2](../10-decisions/adr-084-comercio-dominios-registrar.md#amendments)); `completed`/`failed` los emite el reconcile cron (`advanceTransfer`) | **`completed`** → **`GenerateInvoiceOnDomainTransferCompletedListener`** (**cobro al completar**, billing, [ADR-084 A2.3](../10-decisions/adr-084-comercio-dominios-registrar.md#amendments)) + zona DNS/notifs (T3); `initiated`/`failed` → notifs (T3) | `{ service_id, user_id, fqdn, expires_at? }` | **sí** | 🟡 **`domain.transfer_completed` ✅ emitido** por el reconcile (Outbox, Fase **15D.II.T2c.2**) → factura del transfer; `transfer_initiated`/`transfer_failed` ⬜ (Fase **15D.II.T3**) |
