...
> **Última auditoría:** 2026-05-08 (Sprint 15C Fases 15C.B + 15C.C cerradas + mergeadas a master `156ea35` + `69fed47` — los 2 eventos `service.*` declarados en Fase 15C.A siguen aspiracionales; emisión + listeners llegan en Fases F + H). Historial: 2026-05-07 (Sprint 15C Fase 15C.A — 2 eventos `service.*` aspiracionales declarados en ADR-083 §6 decisión 24 + §4 decisión 14); 2026-05-03 post Sprint 16 cierre Fase 16.E; 2026-04-26 P0.2 (Outbox `invoice.*`); 2026-04-27 Sprint 9 (BullMQ dispatcher + eventos operativos `outbox.event_failed`/`dlq.job_failed`/`system.error`); 2026-04-28 Sprint 9.5 (consumidor `system.error`); 2026-05-01 Sprint 8 Fase C/D (`task.overdue`/`task.unassigned_overdue`/`maintenance.critical` + 4 `support_inside.*`); 2026-05-02 Sprint 11 (5 eventos `service.*` nuevos + consumidor canónico `invoice.paid`).
...
| `service.admin_sso_impersonation` ⭐ | `getSsoUrlWithAudit` cuando `request.actor.role === 'admin'` y service del cliente (Sprint 15C — [ADR-083 §4 decisión 14](../10-decisions/adr-083-plugin-enhance-cp-specifics.md)) | (pendiente listener `audit-on-service-admin-sso-impersonation` Sprint 15C Fase F + portal RGPD `/dashboard/transparency`) | `{ service_id, client_id, agent_user_id, agent_ip, agent_user_agent, panel_label, opened_at, gdpr_visible_to_data_subject: true }` | no | ⬜ aspiracional Sprint 15C (Fase F lo emite; listener llega Fase F mismo o Sprint 12.5) |
| `service.reconciled_external_change` ⭐ | Cron `reconcile-enhance-services` cuando detecta divergencia (Sprint 15C — [ADR-083 §6 decisión 24](../10-decisions/adr-083-plugin-enhance-cp-specifics.md) + DH-INV-6 [ADR-082 §1](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md)) | (pendiente `audit-on-service-reconciled-external-change` Sprint 15C Fase H + `notifications-on-reconciliation-threshold-exceeded` con setting `provisioning.enhance_cp.reconciliation_alert_threshold`) | `{ service_id, plugin_slug, change_type: 'subscription_missing'\|'status_divergence'\|'plan_divergence', expected, actual, detected_at }` | no | ⬜ aspiracional Sprint 15C (Fase H lo emite + listener) |
...
- **`service.cancelled/paused/resumed/suspended` siguen huérfanos** — Sprint 11 los deja huérfanos por diseño: los plugins triviales (`internal`/`manual`) NO necesitan deprovision real (no hay recurso externo). Cuando llegue `docker_engine` (Sprint 15E) o `enhance_cp` (Sprint 15C), el orquestador añadirá el listener `provisioning-on-service-suspended` para invocar `plugin.deprovision()` o equivalente.
