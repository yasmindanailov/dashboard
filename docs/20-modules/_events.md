...
> **Outbox Pattern:** ✅ **8 eventos** — `invoice.created/paid/failed/overdue` + `domain.registered` (15D.D) + `domain.renewed`/`domain.expired`/`domain.entered_redemption` (15D.E). Pendientes `service.*` y `partner.*` (bajo P-DEPLOY) — ADR-033 + auditoría 2026-06-21 (MEDIUM-1: `service.*` ya accionable).
...
> **Emisión faseada.** `domain.registered` ✅ (Fase 15D.D) · `domain.renewed` + `domain.expiring_soon` + `domain.expired` + `domain.entered_redemption` ✅ (**Fase 15D.E**) · gestión `domain.*_changed` (Fase F) · `domain.transfer_*` (Sprint 15D.II). Todos los de **dinero/estado** vía **Outbox** (R8) — emitidos transaccionalmente por el orquestador (renew) o el reconcile cron (expired/redemption); `domain.expiring_soon` es alerta (sin Outbox). Los 4 de 15D.E los consume `NotificationsOnDomainLifecycleListener` (email + campana).
