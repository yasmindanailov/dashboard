...
| `service.activated` | `ProvisioningOrchestratorService.markActive()` (Sprint 11 Fase 11.B — emitido CUANDO `services.status` pasa a `'active'` tras `plugin.provision()` exitoso) | **`ClientLifecycleTaskCreatorListener`** (Sprint 16) → si `clientsService.isFirstService(client_id)`: crea task `client_lifecycle` con SLA 48h. (Plugins reales Fase 11.C/15A-G también consumen este evento, NO `service.provisioned`.) | `{ service_id, user_id, correlation_id }` | no | ✅ con consumidor Sprint 16 (`ClientLifecycleTaskCreatorListener`); plugins reales Sprint 15A-G consumirán cuando lleguen |
...
| `service.suspended` ⭐ | **Dos emisores** (transitorio — ver nota abajo): (1) `ProvisioningService.suspendAsAdmin` (Sprint 15C.II Fase F — [ADR-077 Amendment A4.5](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#a45-materialización-sprint-15cii-fase-f1-2026-05-12)) cuando un admin suspende vía `POST /admin/services/:id/suspend` (tras la inline action `suspend_service` del plugin + `prisma.service.update(status='suspended', suspended_at, suspension_reason)`). **Shape canónico.** (2) `ServiceLifecycleWorker.autoSuspendServices()` (Sprint 6.5 — cron diario 03:00, impago vencido con retries agotados) — payload legacy `{service_id, invoice_id, reason: 'payment_exhausted'}` (sin `user_id`). | **`NotificationsOnServiceSuspendedListener`** (Sprint 15C.II Fase F) → si `notify_client !== false` (el emisor legacy no lo trae → notifica): `dispatchToUser('service.suspended', {domain, reason_label, is_overdue_payment, is_maintenance, billing_url, support_url}, user_id)`. Tolera ambas formas: deriva `user_id` del service si falta; normaliza `reason` legacy → `'overdue_payment'`. Email cliente-seguro (etiqueta localizada del motivo canónico, NUNCA la nota interna) + CTA ramificado por motivo (regulariza pago / soporte / nada para mantenimiento). Patrón L11+L12, degradación elegante R7. | canónico: `{ service_id, user_id, provisioner_slug, reason: SuspensionReason, actor_user_id, suspended_at, notify_client }` · legacy: `{ service_id, invoice_id, reason: 'payment_exhausted' }` | no | ✅ consumido Sprint 15C.II Fase F (listener email + campana) |
| `service.unsuspended` ⭐ | `ProvisioningService.unsuspendAsAdmin` (Sprint 15C.II Fase F — [ADR-077 Amendment A4.5](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#a45-materialización-sprint-15cii-fase-f1-2026-05-12)) cuando un admin reactiva un servicio suspendido vía `POST /admin/services/:id/unsuspend` (tras la inline action `unsuspend_service` del plugin + `prisma.service.update(status='active', suspended_at=null, suspension_reason=null)`). | **`NotificationsOnServiceUnsuspendedListener`** (Sprint 15C.II Fase F) → siempre `dispatchToUser('service.unsuspended', {domain, panel_url}, user_id)` ("tu servicio vuelve a estar activo" — sin toggle de supresión, reactivar es buena noticia). Patrón L11+L12, R7. | `{ service_id, user_id, provisioner_slug, actor_user_id, previous_suspension_reason }` | no | ✅ consumido Sprint 15C.II Fase F (listener email + campana) |
...
- **`service.suspended` con 2 emisores — estado transitorio.** El shape canónico es el de `ProvisioningService.suspendAsAdmin` (con `user_id` + `notify_client`); el emisor legacy `ServiceLifecycleWorker.autoSuspendServices` (impago vencido — Sprint 6.5) emite la forma reducida. **Diferido `DC.NEW-15CII-BILLING-SUSPEND-UNIFY`:** cuando se toque el módulo billing, migrar `autoSuspendServices` para que llame a `ProvisioningService.suspendAsAdmin` con `reason: 'overdue_payment'` + actor "sistema" (en vez de `prisma.service.update` + emit directos) — así emite el shape canónico y reactivación automática al pagar reusa `unsuspendAsAdmin`. Hasta entonces, el listener `notifications-on-service-suspended` tolera ambas formas. Igualmente, `service.resumed` (pause) ↔ `service.unsuspended` (suspend) podrían unificarse — diferido.
