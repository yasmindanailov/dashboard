...
| `domain.transfer_initiated` / `domain.transfer_completed` / `domain.transfer_failed` | FSM de transfer ([ADR-084 §4](../10-decisions/adr-084-comercio-dominios-registrar.md) + [A2](../10-decisions/adr-084-comercio-dominios-registrar.md#amendments), motor = reconcile cron) | notifs, audit, zona DNS en `completed` (capability-routed, [ADR-082 A5](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md#amendments)) | `{ service_id, user_id, fqdn, ... }` | **sí** | ⬜ **mecánica congelada** ([ADR-084 A2](../10-decisions/adr-084-comercio-dominios-registrar.md#amendments), Fase 15D.II.A); emisión Fase **15D.II.T3** |
