...
- **`Notification`** (campana — shape preservado, `read_at NULL` = unread). **F3·E10:** gana `category` (`NotificationCategory`) derivada del `event_type` vía `notification-taxonomy.ts` (fuente única) y persistida por el `InAppChannel` al crear la fila. Cliente y admin comparten el enum; el destinatario decide la bandeja. `general` = fallback.
...
| `GET` | `/api/v1/notifications` | Histórico paginado del usuario (query: `page`, `limit`, `unread_only`, `category`). **F3·E10:** `category` filtra server-side por `NotificationCategory` (correcto con paginación). Alimenta las bandejas full-page `/dashboard/notifications` y `/admin/notifications` | `List.Notification` |
