...
| `provisioning.enhance_cp.reconciliation_alert_threshold` | number | 5 | 🟡 seed Sprint 15C Fase D, consumidor pendiente Fase H | Setting seedeado en Fase D para que esté disponible cuando llegue su consumidor: cron `reconcile-enhance-services` BullMQ cada 6h (Sprint 15C Fase H). Si nº divergencias detectadas / día (events `service.reconciled_external_change` agregados) > threshold → listener `notifications-on-reconciliation-threshold-exceeded` alerta superadmin (canal internal + email). DH-INV-6 doctrine (ADR-082 §1) — Aelium adopta cambios externos pero notifica si el flujo se vuelve patológico (operator pisando Aelium repetidamente). | [ADR-083](../10-decisions/adr-083-plugin-enhance-cp-specifics.md) §6 decisión 24 + §"Settings canónicos NUEVOS" · seed Sprint 15C Fase D (PR #41, master `a319063`) — consumidor Fase H |
