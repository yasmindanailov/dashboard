...
| 15C.E | Acciones curadas (reset_password + view_disk + view_bandwidth + change_package admin + force_resync admin) + audit completo + flag canónico `ServiceAction.adminOnly` (ADR-077 A3) + 10ª action `list_available_plans` (ADR-083 A3) + enforcement HTTP 403 + evento `service.action_admin_only_violation` | 0.5–1 sesión | 🔄 branch local `sprint15c-fase-e-acciones-curadas` (4 commits encadenados) — pendiente PR/merge (suite 454/459 + 5 skipped + lint:check + build verde) |
