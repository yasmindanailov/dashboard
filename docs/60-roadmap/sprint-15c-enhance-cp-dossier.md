...
| 15C.E | Acciones curadas (reset_password + view_disk + view_bandwidth + change_package admin + force_resync admin) + audit completo + flag canónico `ServiceAction.adminOnly` (ADR-077 A3) + 10ª action `list_available_plans` (ADR-083 A3) + enforcement HTTP 403 backend + evento `service.action_admin_only_violation`. **Solo backend** — frontend filter por `adminOnly` queda aspiracional (ADR-077 A3.5) hasta que se cree la primera UI admin que invoque acciones admin-only (la página `/admin/services/[id]` aún no existe). | 0.5–1 sesión | 🔄 PR [#44](https://github.com/yasmindanailov/dashboard/pull/44) — 5 commits + 1 commit fix-doc + suite 454/459 + 5 skipped + lint:check + build verde |
...
| **DC.NEW-15C-FORM** ⭐ (descubierto Fase 15C.E review 2026-05-09) | Form admin de productos (`frontend/app/admin/products/new/page.tsx` + `[id]/edit/_components/ProductEditForm.tsx`) **NO expone `provisioner_config`** — solo ingresa el slug del provisioner como texto libre. Backend DTO ([`product.dto.ts:224`](../../backend/src/modules/products/dto/product.dto.ts#L224)) sí acepta el campo `provisioner_config: Prisma.InputJsonValue`. Sin UI, un producto creado con `provisioner='enhance_cp'` queda con `provisioner_config: null` y al provisionarlo `enhance.plugin.ts:330` lanza `INVALID_PAYLOAD: enhance_plan_id missing` no-retriable → service queda en `cancelled`. Solución canónica: sub-form dinámico por provisioner que renderice schema JSON-Schema 7 (similar al patrón Sprint 15A `@rjsf/core` para plugin install) — para `enhance_cp` campo `enhance_plan_id: integer`. | Antes de poder contratar producto Enhance real (bloqueante operativo). Se aborda como **Sprint 15C.E.2** (UI extension) o se difiere a Sprint 12 (Settings + UI dinámicos) según prioridad comercial. |
| **DC.NEW-15C-PLUGIN-SEED** ⭐ (descubierto Fase 15C.E review 2026-05-09) | Plugin install `enhance_cp` **NO se sieman** en `pnpm seed` ([`plugin-installs.ts:30-33`](../../backend/prisma/seeds/plugin-installs.ts#L30-L33) — comentario L19 declara que plugins reales se instalan en runtime via UI). Para QA/staging/dev sería útil un seed condicional `NODE_ENV !== 'production'` que pre-cree el plugin install con baseUrl + masterOrgId desde env vars + secret apiToken desde env var dedicada → ahorro de configuración manual repetitiva. **No bloqueante** — el admin lo configura una vez via UI y persiste. | Sub-sprint dedicado de DX (developer experience) o como parte de Sprint 15C.I cierre. |
| **DC.NEW-15C-FILTER** ⭐ (descubierto Fase 15C.E review 2026-05-09) | Frontend `ActionsBar` ([`frontend/app/_shared/services/ActionsBar.tsx:66-75`](../../frontend/app/_shared/services/ActionsBar.tsx#L66-L75)) **NO filtra `availableActions` por `adminOnly`** — renderiza todas. Cliente final ve los 3 botones admin-only de Enhance (`change_package`, `force_resync`, `list_available_plans`); al pulsarlos recibe HTTP 403 (defense-in-depth backend funciona, [ADR-077 A3.5 patrón aspiracional](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#amendments)). UX subóptima — los botones no deberían aparecer al cliente. Solución: añadir `useAuthContext` o pasar `isAdmin` como prop + filter `actions.filter(a => !a.adminOnly \|\| isAdmin)` antes de map. | Junto con primera UI admin que necesite acciones admin-only operables (Sprint 15C.G frontend DNS UI sentará el patrón si introduce admin-only DNS actions, o fase posterior cuando se cree `/admin/services/[id]`). |
| **DC.NEW-15C-ADMIN-PAGE** ⭐ (descubierto Fase 15C.E review 2026-05-09) | Página `/admin/services/[id]` **NO existe** en frontend. `frontend/app/admin/services/` solo tiene `page.tsx` (lista) + `_components/AdminServicesView.tsx`. Admin solo puede ver detalle vía path cliente con `getInfoForUser(serviceId, userId, isAdmin=true)` (bypass ownership en `ProvisioningService:175`). Modal admin `change_package` (decisión 30 ADR-083) inalcanzable hoy desde UI. Solución: SC nativo paralelo a `/dashboard/services/[id]` con `info` enriquecido + acciones admin desbloqueadas + modal `change_package` que invoca first `list_available_plans` (10ª action, populates dropdown) luego `change_package` con planId elegido. | Fase frontend dedicada — no programada en current.md Sprint 15C ni en backlog P2. Aspiración Sprint 12 (Settings + UI dinámicos) o nuevo Sprint 15C-frontend dedicado. |
| **DC.NEW-15C-E2E** ⭐ (descubierto Fase 15C.E review 2026-05-09) | **No hay E2E spec** que ejercite el flujo completo Enhance: producto admin con `provisioner_config.enhance_plan_id` → cliente checkout → `invoice.paid` → orchestrator `provision()` 6-step contra mock → `service.activated` → cliente abre `/dashboard/services/[id]` → 10 botones renderizados → click `view_disk_usage` → 200 + métricas; click `change_package` cliente → 403 + audit; click admin → 200 + plan changed. Solo cobertura unit + integration cliente↔mock. | Sprint 15C Fase 15C.I cierre — declarado en dossier §7 fila I: *"E2E (mock server completo + smoke contra live) + cierre documental + retrospectiva + i18n strings (1 sesión)"*. Esta DC formaliza el alcance E2E concreto que la Fase I debe cubrir. |
