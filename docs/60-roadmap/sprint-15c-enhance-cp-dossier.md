...
| **15C.H** ⚡ SIGUIENTE | **Reconciliation L3 cron + listener audit + notificación threshold**. Materializa [ADR-083 §6 decisión 24](#6-reconciliation-3-capas-decisiones-22-24). **Cron BullMQ scheduled `reconcile-enhance-services`** ejecuta cada 6h (configurable via `manifest.configSchema.reconciliationIntervalHours` ya seedeado default `6`). Itera services con `provisioner_slug='enhance_cp' AND status IN ('active','suspended')` invocando `plugin.getStatus(service)` (ya implementado Fase C). Según resultado:<br/>· `subscription_missing` (404 Enhance) → emite `service.reconciled_external_change` con `change_type='subscription_missing'`. Aelium marca `Service.status='unknown'` (NO `'cancelled'` automático — DH-INV-6 + dossier §6.10).<br/>· `status_divergence` (Enhance status ≠ Aelium status) → emite con `change_type='status_divergence'` + Aelium **adopta** estado Enhance (DH-INV-6 → Enhance gana).<br/>· `plan_divergence` (`Subscription.planId ≠ Product.provisioner_config.enhance_plan_id`) → emite con `change_type='plan_divergence'` (NO auto-corrige — billing implication, admin decide).<br/><br/>**Listeners nuevos:**<br/>· `AuditOnServiceReconciledExternalChangeListener` — persiste en `audit_change_log` con flag GDPR opcional según `change_type` (status_divergence visible al cliente; plan_divergence solo admin).<br/>· `NotificationsOnReconciliationThresholdExceededListener` — cuenta divergencias / día contra setting `provisioning.enhance_cp.reconciliation_alert_threshold` (ya seedeado Fase D, default `5`). Si supera → notif superadmin canónica.<br/><br/>**Plan paso a paso para próximo agente** (orden canónico):<br/>1. Pre-research: leer ADR-083 §6 decisión 24 + dossier §6.10 + estado código (`getStatus()` plugin enhance ya devuelve `{status, statusReason, checkedAt}`; setting threshold ya existe). Identificar dónde vive el cron (patrón canónico: `backend/src/plugins/provisioners/enhance_cp/crons/` siguiendo el patrón `audit-retention.cron.ts` con `@Cron(...)` decorator + ScheduleModule).<br/>2. Backend: `EnhanceReconciliationCron` con `@Cron(CronExpression.EVERY_6_HOURS)` (o configurable via setting) + lógica iterativa con `prisma.service.findMany({where: {provisioner_slug: 'enhance_cp', status: { in: ['active', 'suspended'] }}})`. Por cada service: invoca `plugin.getStatus(service)` + compara con `Service.status` Aelium + compara `enhance_plan_id` cacheado. Emite eventos según resultados. Aplica DH-INV-6 (status: adopta; plan: solo emit). Persiste lastReconciledAt si necesario (¿columna nueva en `services`? — decidir; sugerencia: NO añadir columna, basta con timestamp del último audit log row).<br/>3. Listener `AuditOnServiceReconciledExternalChangeListener` (en `modules/audit/listeners/` o similar — alinearse con patrón Fase F `AuditAdminSsoImpersonationListener`). Mapea `change_type` → flag GDPR. Persiste con `metadata.target_user_id = service.user_id` para que `/dashboard/transparency` lo exponga al cliente cuando aplique.<br/>4. Listener `NotificationsOnReconciliationThresholdExceededListener` (en `modules/notifications/listeners/` siguiendo el patrón). Counter Redis o query SQL para contar `audit_access_log WHERE action='reconciled_external_change' AND created_at > now() - interval '1 day'`. Si supera threshold → emit `notifications.send` canónico al superadmin.<br/>5. Tests: spec del cron (mock plugin getStatus + verificar 3 escenarios drift) + spec de cada listener + invariante test contract (cron registrado en módulo).<br/>6. Doc canónica: `_events.md` actualizar fila `service.reconciled_external_change` (estado ⬜ aspiracional → ✅ consumido), `provisioning/contract.md` §Emite, `features/services/admin.md` §8 Eventos, `features/services/client.md §7.5.X` (qué ve el cliente cuando hay drift detectado vs su servicio).<br/>7. Smoke: insertar audit row sintética via psql (similar a Fase F smoke 1) + verificar transparency cliente; o orquestar cron `runOnce()` manualmente.<br/><br/>**Ambigüedades doctrinales para preguntar a Yasmin antes de codear:**<br/>· **(A1)** ¿Cron decorator estático (`@Cron('0 */6 * * *')`) o dinámico vía `SchedulerRegistry` leyendo `reconciliationIntervalHours` runtime? Recomendación: estático — el setting es config inicial; cambiar el intervalo requiere redeploy es aceptable v1.<br/>· **(A2)** ¿`status_divergence` adopta automáticamente o crea task admin? Recomendación: adopta automáticamente (DH-INV-6 literal). Audit pesado deja trazabilidad.<br/>· **(A3)** Counter del threshold: ¿Redis-based (rápido) o SQL count cada vez (más simple)? Recomendación: SQL — el cron es 6h, no hay presión de latencia. | 0.5 sesión | ⏳ pendiente — siguiente fase tras merge PR #47 |
