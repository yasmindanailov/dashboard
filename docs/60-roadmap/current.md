...
| 15D.II.T2c.3 | Transfer entrada (UX) | **Decisión Yasmin: CARRITO ÚNICO** (como register). Plan: el checkout flagea los ítems `transfer_in` como **deferBilling** (se crean `pending` pero **se excluyen de la factura** — el cobro lo hace T2c.2 al completar; factura nullable si el carrito es solo-transfers) + **endpoint** `POST /domains/:id/transfer/submit-auth {authCode}` → `initiateTransferIn` (el auth-code se aporta **post-checkout**, fuera del carrito: secreto fuera + no bloquea el checkout en la API RC). Toca el checkout central (return) + endpoint nuevo + wiring + frontend. | ⬜ **(siguiente)** |
| 15D.II.T3 | Transfer cierre | Eventos `domain.transfer_*` vía Outbox + notifs + zona DNS al completar (capability-routed). | ⬜ |
