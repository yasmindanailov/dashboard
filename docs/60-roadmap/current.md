...
> Cabeza de cola activa P2.3 — primer plugin real post Sprint 15A. Rama `sprint15c-plugin-enhance-cp` creada desde `master@80492ad` (post merge dossier 15C PR #35). 9 fases planificadas (15C.A → 15C.I) — 7-10.5 sesiones según [`sprint-15c-enhance-cp-dossier.md` §7](./sprint-15c-enhance-cp-dossier.md#7-estimación-esfuerzo-sprint-15c--9-fases). Bloqueante operacional para Sprint 15D RC (sin DNS authority cluster Enhance, los dominios registrados con NS=Aelium quedarían sin destino).

### Fase 15C.A — ADRs frozen (en curso)

3 ADRs doc-only redactados en este Sprint para congelar la doctrina antes del primer commit funcional:

| ADR | Status | Contenido |
|---|---|---|
| [ADR-082](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md) — Modelo Domain↔Hosting + DNS doctrine (transversal) | ✅ redactado | 6 invariantes DH-INV-1..6 (DH-INV-6 ⭐ Enhance gana en conflicto) + 4 flujos checkout F1-F4 + DNS-as-capability + NS-sync 3 capas (C1/C2/C3) + listener reconcile defensivo + cross-plugin DNS authority resolver. Consumido por todos los hosting/registrar plugins futuros. |
| [ADR-077 Amendment A1](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#amendments) — capability `has_dns_management` | ✅ redactado | Añade flag booleano required a `PluginCapabilities` + actualiza mapping inicial + amplía test contract genérico con 3 invariantes + actualiza plugins existentes (`internal`, `manual` → `false`). Compatible hacia atrás (no bumpea contractVersion). |
| [ADR-083](../10-decisions/adr-083-plugin-enhance-cp-specifics.md) — Plugin Enhance CP specifics | ✅ redactado | 35 decisiones frozen: auth bearerAuth Super Admin + `secretsSchema` apiToken + tabla nueva `enhance_customers` (PK natural client_id) + lazy create idempotente con search-by-email + provision 6-step (customers/logins/members/owner/subscriptions/websites) + SSO 2-call OTP + DNS authority/records (delega §3-§6 a ADR-082) + reconcile 3 capas (60s/on-demand/6h) + MockEnhanceServer + plan upgrade admin-only + DH-INV-6 operational doctrine + 2 eventos canónicos nuevos (`service.admin_sso_impersonation`, `service.reconciled_external_change`) + 2 settings nuevos (`provisioning.default_nameservers`, `provisioning.enhance_cp.reconciliation_alert_threshold`). |

### Fases siguientes (planificadas)

- **15C.B** — `EnhanceApiClient` HTTP + types TypeScript + `MockEnhanceServer` Express + captura fixtures contra live (0.5-1 sesión).
- **15C.C** — Plugin core (6 métodos contrato + manifest + DI registration + tabla `enhance_customers` Prisma + lazy-create idempotente) (1-1.5 sesión).
...
- **15C.I** — E2E (mock server completo + smoke contra live) + cierre documental + retrospectiva (1 sesión).
