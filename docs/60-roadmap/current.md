...
> **Última actualización:** 2026-06-22 — **🚀 Sprint 15D (ResellerClub + comercio de dominios) EN CURSO.** Fases **15D.A→E ✅** (A→D mergeadas, master `ae70010`/PR #108; **15D.E en rama `sprint15d-fase-e-renew-lifecycle`**): doctrina → fundación+contrato → cliente HTTP RC + mock → plugin core (`provision(register)` + DOM-INV-1 + `domain.registered`) → **renovación (`provision(renew)` + DOM-INV-4) + lifecycle de expiración + 3 crons (reconcile/pricing-sync/avisos) + eventos `domain.*` + spec de BillingInvoiceService**. **Próxima: 15D.F** (gestión curada `executeAction` + zona DNS post-register capability-routed + buscador + frontend). Detalle por fase en §Sprint 15D abajo.
>
...
| 15D.F | Gestión + buscador | Acciones curadas (NS/contactos/privacy/lock/auth-code) + admin suspend/unsuspend + buscador/availability + **zona post-register vía orquestador** *(capability-routed por DNS-authority — [ADR-082 A3](../10-decisions/adr-082-modelo-domain-hosting-dns-doctrine.md#amendments)/DH-INV-7, cero acoplamiento a `enhance_cp`)* + audit | ⬜ |
