...
| **8.D** | **Support Inside (UX dedicada — [ADR-061](../10-decisions/adr-061-support-inside-tier-cuenta-ux.md) + [ADR-075](../10-decisions/adr-075-support-inside-ux-lista-y-aislamiento-productos.md))** — sub-sprint denso, ~2 sesiones | ⬜ |
...
| 8.D.4 | Endpoints admin `/api/v1/admin/support-inside/plans` (3): `GET` listado, `GET /:slug` detalle full, `PATCH /:slug` actualiza en transacción (products + support_inside_config + product_pricing). NO POST/DELETE — los 3 planes son seedeados; cuarto plan exige migración + ADR (ADR-075 §A.3). CASL `Manage.SupportInside` (subject nuevo, sólo `superadmin` + `agent_full`) | ⬜ |
| 8.D.4b | **[ADR-075]** Aislamiento del CRUD genérico de productos: (a) `AdminProductsController.create/update/delete` rechaza `type='support_inside'` con 400 salvo header interno `X-Aelium-Source: support-inside-admin`. (b) Frontend constante canónica `PRODUCT_TYPES_CREATABLE` excluye `support_inside`. (c) Listado `/admin/products` renderiza filas `support_inside` en gris con badge "Tier de cuenta" + link a página dedicada. (d) Detalle directo `/admin/products/:id` con `type='support_inside'` redirige a `/admin/support-inside-plans/<slug>` con toast | ✅ (a backend Sprint 8 Fase D backend; b/c/d Sprint 8 Fase D frontend 2026-05-01) |
| 8.D.5 | Cliente: página `/dashboard/support-inside` — formato comparador 3 cards lado a lado (target: cliente que decide qué contratar). Si tiene plan activo: vista de gestión (slots + servicios asignados + historial valor). | ✅ Sprint 8 Fase D frontend (2026-05-01) |
| 8.D.6 | **[ADR-075]** Admin: índice `/admin/support-inside-plans` con tabla vertical de 3 filas (nombre, precio mensual, precio anual, slots, activo, última edición). NO formato comparador. Click en fila → editor `/admin/support-inside-plans/<slug>` | ✅ Sprint 8 Fase D frontend (2026-05-01) |
| 8.D.6b | **[ADR-075]** Admin: editor `/admin/support-inside-plans/<slug>` con secciones card extensibles. 5 secciones canónicas Sprint 8: Identidad · Precios · Slots y capacidades · Soporte y canales · Configuración avanzada. Cada card con botón "Guardar cambios" propio (sin auto-save) + warning navegación si dirty. Patrón canónico: cada sprint futuro añade UNA card nueva al final, NO redistribuye | ✅ Sprint 8 Fase D frontend (2026-05-01) |
| 8.D.7 | `MaintenanceMonthlyCron` (BullMQ scheduled cron `0 6 1 * *` UTC — replicar patrón Fase C) — genera `Task(type=maintenance_management)` por slot activo. Idempotente por `(service_id, billing_month)`. Endpoint admin trigger manual `POST /admin/tasks/cron/maintenance-monthly` para smoke + E2E | ⬜ |
| 8.D.8 | Cancelación cascada de slots cuando se cancela subscription. Servicios técnicos del cliente se mantienen intactos (sólo se desactiva el slot Support Inside). | ⬜ |
| 8.D.9 | Seed: 3 planes Básico/Medium/Pro con `support_inside_config` poblado + `product_pricing` (mensual + anual). Campos canónicos del editor pre-poblados. Idempotente por `slug` | ⬜ |
| 8.D.10 | Tests E2E `tests/e2e/support-inside.spec.ts`: (a) cliente subscribe → assign slot → maintenance auto-creada vía cron manual → admin completa con checklist; (b) admin lista `/admin/support-inside-plans` ve 3 filas; (c) admin edita plan Pro sección Precios → cambio refleja en `product_pricing` sin afectar suscripciones activas; (d) `POST /admin/products` con `type='support_inside'` sin header interno → 400; (e) cliente intenta `GET /admin/support-inside/plans` → 403 (defense in depth) | ⬜ |
| **8.D.12** | **Visibilidad transversal Support Inside + cableado horizontal + drift cron + UX asignación slot + refactor checkout vía evento + seed Carla activa** — sub-fase añadida 2026-05-01 tras auditoría que detectó que Support Inside era módulo aislado, NO capa transversal. Cierra los gaps ALTA de la auditoría + 4 drifts canónicos (anniversary_day, services.credit_balance_eur ausente, tasks.slot_id FK pendiente, listeners support_inside.* huérfanos). | ✅ Sprint 8 Fase D.12 (2026-05-01) |
| 8.D.12.1 | **Drift `anniversary_day`**: añadir campo a `support_inside_slots` (`integer NOT NULL CHECK 1..28`), backfill `LEAST(EXTRACT(DAY FROM assigned_at), 28)`, idx canónico `idx_slots_anniversary`. Migración Prisma `sprint8d12_anniversary_day`. Reescribir `MaintenanceMonthlyService.generateMonthly()` para filtrar `WHERE anniversary_day = EXTRACT(DAY FROM NOW())` + cron pasa de `0 6 1 * *` a `0 6 * * *` (diario, ADR-034 §recurrencia "distribuir carga"). Tests unit del filtro + idempotencia `(service_id, billing_month, type)` sigue intacta. | ✅ |
| 8.D.12.2 | **Listener `support-inside-priority`**: `@OnEvent('conversation.created')` lee la suscripción activa del `client_id`; si existe, sobrescribe `priority` del ticket/chat con `support_inside_config.priority_tier` mapeado (`standard→normal`, `high→high`, `max→urgent`). Defense in depth: solo si la conversación NO especifica `priority` explícito (preserva la elección manual del agente). Test E2E: cliente con Pro abre ticket → priority=urgent automático. | ✅ |
| 8.D.12.3 | **Listener `support-inside-audit`**: `@OnEvent('support_inside.subscribed\|cancelled\|slot_assigned\|slot_released')` invoca `AuditService.logChange(actor, 'support_inside_subscription'\|'support_inside_slot', before, after)`. Cumple R3 (audit inmutable) y materializa la doctrina ADR-061 §"tier de cuenta visible" en el portal de transparencia cliente (Sprint 9 Fase E). | ✅ |
| 8.D.12.4 | **Helper transversal `clientsService.findOne` enriquecido**: incluye `support_inside_subscription { status, product { name, slug }, support_inside_config { priority_tier, response_sla_hours, channels_active } }` en la respuesta admin. Sin N+1 — single query con `include`. Documentado en `clients/contract.md` §respuesta admin. | ✅ |
| 8.D.12.5 | **Frontend badge `ClientDetailHeader` admin**: cuando `client.support_inside_subscription.status === 'active'`, render `<Badge variant="brand">Support Inside {tier}</Badge>` + tooltip "SLA {N}h · canales: {lista}". Patrón canónico para futuras integraciones de tier (Premium, Enterprise, etc.). | ✅ |
| 8.D.12.6 | **Frontend badge `ConversationHeader`**: si la conversación pertenece a cliente con Support Inside, render badge tier+SLA junto al status. Sirve al agente para entender el SLA esperado de un vistazo (auditoría #3). | ✅ |
| 8.D.12.7 | **Frontend stat `/dashboard` overview cliente**: `ClientStats` añade card 5 "Mi plan Support Inside" con plan_name + "{slots_used}/{slots_included} slots usados" + link a `/dashboard/support-inside`. Si no tiene plan: card "Activa Support Inside" con CTA. | ✅ |
| 8.D.12.8 | **UX asignación slot desde `/dashboard/support-inside` (vista gestión)**: botón "Asignar slot" abre modal con `<Select>` de servicios `active` del cliente sin slot activo. Tipo de slot pre-seleccionado al primero permitido por el plan. Llama `POST /dashboard/support-inside/slots` (endpoint ya existente). Entrada simétrica desde `/dashboard/services` queda formalmente diferida a Sprint 11 Provisioning (no existe el directorio aún, doctrina ADR-070 dependiente). | ✅ |
| 8.D.12.9 | **Refactor checkout vía evento (ADR-076 nuevo)**: eliminar el modal de subscribe en `/dashboard/support-inside`. El comparador redirige a `/dashboard/billing/checkout?product_pricing_id={pricingId}` (página YA existente). Cuando `BillingCheckoutService.checkout()` resuelve un producto `type='support_inside'`, emite `service.provisioned` que `support-inside-on-service-provisioned.listener` consume para crear/reactivar la `SupportInsideSubscription`. Endpoint `POST /dashboard/support-inside/subscribe` queda como API alternativa interna (no expuesta en frontend) para compatibilidad y tests E2E. **ADR-076** documenta: "un único motor de checkout por dominio billing — Support Inside no es excepción, es consumidor de eventos service.*". Cuando llegue Stripe (post-Sprint 14), el flujo 3D Secure entra una sola vez en `/dashboard/billing/checkout`. | ✅ |
| 8.D.12.10 | **Seed `cliente@aelium.test` activa Plan Medium + 1 slot**: nueva función `seedSampleSupportInsideSubscription(prisma)` en `prisma/seeds/sample-support-inside.ts` que: (a) localiza el cliente Carla (idempotente por email), (b) si no tiene subscription activa, ejecuta el flujo canónico vía `BillingCheckoutService.checkout()` + listener D.12.9 (no atajo Prisma) para que el seed entrene el flujo real, (c) asigna 1 slot al primer servicio `active` del cliente (si no tiene servicios, no asigna slot — degradación elegante). Guard `NODE_ENV !== 'production'` igual que el resto de demo data. Idempotente — `pnpm seed` repetido NO duplica subscriptions ni slots. Marker `metadata.seeded = true` en subscription. | ✅ |
| 8.D.12.11 | **Tests + DoD**: +6 unit tests (`support-inside-priority.listener` × 3 con priority/sin priority/cliente sin SI · `support-inside-audit.listener` × 2 · cron filtro `anniversary_day` × 1) + 2 E2E (`tests/e2e/support-inside-transversal.spec.ts`: badge en ticket de cliente con Pro · stat overview muestra plan). Suite full ≥143/143 unit verde + ≥119/119 E2E. | ✅ |
...
| EC-T8-47 | Cliente con Support Inside activo abre ticket — el listener D.12.2 sobrescribe `priority` aunque el agente quiera dejarla en `normal` | Sprint 8 Fase D.12.2 — el listener SOLO actúa si `dto.priority` viene `null/undefined`, no pisa la elección manual del cliente/agente. Test E2E. |
| EC-T8-48 | Cron diario `0 6 * * *` se dispara para `anniversary_day=29..31` que NO existe en febrero | Sprint 8 Fase D.12.1 — CHECK constraint 1..28 en BD impide guardar `>28`. Cron filtra por `EXTRACT(DAY FROM NOW())` que en feb 1-28 cubre todo. Test unit. |
| EC-T8-49 | Cliente cambia `subscription` a otro plan mid-mes (workaround "cancela y recontrata") y los slots se pierden silenciosamente | Sprint 8 Fase D.12 — la cancelación cascada YA libera slots (cerrado D.2). Documentar en `support-inside/contract.md` que el cliente debe re-asignar slots tras recontratación. Cierre del gap completo (upgrade prorrateado real) queda como deuda P1 transversal con ADR-077. |
| EC-T8-50 | Listener `support-inside-on-service-provisioned` (D.12.9) recibe el evento pero el `service.product.type !== 'support_inside'` (cualquier servicio normal) | Listener filtra explícitamente al inicio: `if (payload.product.type !== 'support_inside') return;`. Sin esto el listener intentaría crear subscriptions de hosting/dominio. |
| EC-T8-51 | `BillingCheckoutService.checkout()` falla a mitad (ej. PROD-INV-5) y el evento `service.provisioned` no se emite — la suscripción Support Inside queda sin crearse pero la factura del cliente sí | Sprint 8 Fase D.12.9 — `service.provisioned` se emite **dentro de la transacción** que crea Service+Invoice (R8 Outbox cuando se extienda — P-DEPLOY.4). Si falla antes del emit, no hay factura tampoco — invariante coherente. |
| EC-T8-52 | Seed `pnpm seed` ejecuta D.12.10 sobre Carla pero ella aún no tiene servicios `active` (entorno fresco) | Seed degrada elegante: crea subscription pero NO asigna slot. Log info. El smoke testing manual (Yasmin) verifica que las dos rutas (con servicio / sin servicio) funcionan tras seed. |
...
#### ADRs nuevos confirmados (Sprint 8 Fase D.12 — añadido 2026-05-01)

- **ADR-076 — Checkout único por dominio billing (Support Inside es consumidor de eventos `service.*`)**: formaliza la decisión de eliminar el endpoint dedicado `POST /dashboard/support-inside/subscribe` como entrada UX y migrar al flujo canónico `/dashboard/billing/checkout`. La creación de la `SupportInsideSubscription` pasa a ser efecto colateral de `service.provisioned` consumido por listener `support-inside-on-service-provisioned`. Razones: (a) un único motor de billing para integrar Stripe/Redsys post-Sprint 14, (b) cumple R1 (módulos por eventos) por construcción, (c) elimina asimetría UX modal-vs-redirect, (d) `/dashboard/billing/checkout` ya soporta selector de `billing_profile_id` que el modal no soportaba. ADR-061 sigue vigente, este ADR solo refina "cómo se materializa el subscribe".

#### Deudas transversales documentadas en backlog (NO entran en 8.D.12)

> 🚨 **Drifts canónicos detectados durante auditoría 2026-05-01**. Cada uno tiene impacto que excede Support Inside — son deudas transversales del dashboard. Se mueven a `backlog.md` con prioridad explícita.

- **DC.16 — `services.credit_balance_eur` ausente en schema** ([ADR-029 §"Sin devolución de dinero"](../10-decisions/adr-029-prorrateo-cambio-plan.md)): el ADR-029 declara este campo como vehículo del prorrateo cuando el crédito sobrante no encaja en el nuevo plan. **No existe en `services` hoy**. Bloquea cualquier flujo upgrade/downgrade real (Support Inside, hosting, etc.). Migración + actualización de `BillingCalculatorService.calculateProration()` para consumir/escribir el saldo. **P1 transversal**.

- **DC.17 — `tasks.slot_id` FK pendiente** (`30-data/support.md:193` lo declara, schema real no lo tiene): cuando se cierre la asignación slot↔servicio (Sprint 11 Provisioning + 8.D.12.8), las tareas de mantenimiento generadas por slot necesitan referenciarlo. Migración `ALTER TABLE tasks ADD COLUMN slot_id uuid NULL REFERENCES support_inside_slots(id) ON DELETE SET NULL` + actualizar `MaintenanceMonthlyService` para setearlo. **P1 dependiente Sprint 11**.

- **DC.18 — Upgrade entre planes distintos del mismo dominio** (ADR-077 propuesto): ADR-029 cubre solo mensual↔anual del MISMO `product_id`. El cambio Básico→Pro de Support Inside, o `hosting-web-pro`→`hosting-web-business`, no tiene flujo canónico. Workaround actual: cancelar y recontratar (pierde días pagados, slots, etc.). ADR-077 a redactar: lógica de migración + prorrateo de la diferencia. **P1 transversal**, bloquea cierre comercial Support Inside y upselling hosting.

- **DC.19 — Slots adicionales facturables como `support_addon`** (ADR-077.b o ADR independiente): hoy `addSlot(is_extra=true)` crea row sin facturar. ADR-034 §sistema de slots declara que los slots adicionales son productos `support_addon` independientes. Implementación canónica: cada slot extra crea su propia fila en `services` con `pricing_id` apuntando a un producto type=`support_addon`. **P1 dependiente DC.16+DC.18**, post-cierre 8.D.12.

- **DC.20 — Channels alternativos (WhatsApp, SMS) en `NotificationsService`** (ADR-061 §canales): `support_inside_config.channels_active` se guarda y se muestra al cliente, pero `NotificationsService` solo despacha por `EmailChannel`+`InAppChannel`. Falta canal `WhatsAppChannel` (Twilio o equivalente) + plantillas + `NotificationsService.dispatchToUser` lee `channels_active` del cliente con SI activo. **P2 — Sprint 12 Settings+KB o sprint dedicado**.

- **DC.21 — Historial de valor cliente Support Inside** (ADR-061 §UX cliente): "consultas resueltas · tiempo medio respuesta · soluciones aplicadas · mantenimientos realizados con detalle" no implementado. Requiere queries cross-módulo (support + tasks + client_notes) + sección dedicada en `/dashboard/support-inside`. **P2** — sprint propio dedicado a métricas tras visibilidad transversal cerrada.

- **DC.22 — Settings globales `/admin/settings/support-inside`** (ADR-061 §Settings globales): 1/3 (`support.maintenance_critical_threshold_days` ya existe). Faltan: configuración de horario de soporte (Sprint 7.6 ops aspiracional) + plantillas respuesta automática fuera de horario. **P2 — Sprint 12 Settings+KB**.

- **DC.23 — Bundles producto+SI** (ADR-061 §"Cuándo revisar"): "Hosting Web Pro + Support Inside Pro como bundle único" no diseñado. Trigger condicional, no sprint asignado.

- **DC.24 — Listado admin clientes con columna/badge Support Inside** (auditoría #2): `/admin/clients` no muestra qué clientes tienen SI activo. Requiere extender `clientsService.findAll` con `include support_inside_subscription` + columna en frontend `ClientsListPage`. **P2 mejora UX admin**, no bloquea operación.

- **DC.25 — Card servicio cliente "Cubierto por Support Inside"** (auditoría #8): cuando exista `/dashboard/services` (Sprint 11), añadir badge en cada card si `service.support_inside_slots.some(s => !s.released_at)`. **Sprint 11 Provisioning**.

- **DC.26 — Topbar/avatar indicador SI** (auditoría #9): icono escudo dorado pequeño junto a la campana cliente si SI activo. **P3 — pulido visual**, no bloquea.

---

### 10. Cierre del sprint

> Sprint 8 sigue **WIP**. **Fase B + Fase C + Fase D backend 100% cerradas** (B.1..B.10 + fix + fix2 + C.0..C.5 + D.1..D.10 backend). Cola restante: **Fase D frontend** (8.D.5 + 8.D.6 + 8.D.6b — cliente comparador, admin índice, admin editor con secciones card extensibles + filtro/redirect ADR-075) → Fase E docs canónicas + retrospectiva.
...
| 8.D.0 (ADR-075 + actualizar ADR-061 nota revisión + plan Fase D refinado en current.md §5 + products/contract.md §17 deuda) | 2026-05-01 | _(este sprint)_ |
| 8.D.1 (Schema Prisma 3 tablas + 5 enums + relaciones inversas User/Service/Product + migración `sprint8d_support_inside_schema` aplicada) | 2026-05-01 | _(este sprint)_ |
| 8.D.2 + 8.D.8 (SupportInsideService: subscribe reusa BillingCheckoutService ADR-061 + cancel cascada de slots + addSlot/releaseSlot/getStatus + upgrade MVP rechazo ADR-029 pendiente) | 2026-05-01 | _(este sprint)_ |
| 8.D.3 + 8.D.4 (Cliente: 6 endpoints `/dashboard/support-inside/*` + DTOs + CASL `SupportInside.Update` ownership · Admin: 3 endpoints `/admin/support-inside/plans*` con DTO de 5 secciones extensibles + CASL `Manage.SupportInside`) | 2026-05-01 | _(este sprint)_ |
| 8.D.4b (SupportInsideIsolationGuard backend: bloquea POST/PATCH/DELETE sobre `type=support_inside` salvo header `X-Aelium-Source: support-inside-admin` — ADR-075 §A.2 defense in depth) | 2026-05-01 | _(este sprint)_ |
| 8.D.7 (MaintenanceMonthlyService + Processor BullMQ scheduled `0 6 1 * *` UTC + endpoint admin trigger manual + idempotencia P2002 por UNIQUE `(service_id, billing_month, type)` + ADR-072 cola pública `assigned_to=null`) | 2026-05-01 | _(este sprint)_ |
| 8.D.9 (Seed canónico 3 planes Básico/Medium/Pro con `support_inside_config` + pricing mensual+anual con 15% descuento + idempotente por slug + integrado en `seed.ts` orquestador) | 2026-05-01 | _(este sprint)_ |
| 8.D.9.fix (Migración `sprint8d_cleanup_legacy_support_inside_basic` versionada elimina `slug=support-inside-basic` huérfano del seed sample previo — limpieza profesional vía Prisma migrate, no SQL ad-hoc) | 2026-05-01 | _(este sprint)_ |
| 8.D.10 + 8.D.11 (Tests E2E `support-inside.spec.ts` 5/5 verde + 28 tests unit nuevos: 15 SupportInsideService + 7 MaintenanceMonthlyService + 6 SupportInsideIsolationGuard; suite full **117/117 E2E + 135/135 unit verde sin regresión**) | 2026-05-01 | _(este sprint)_ |
| 8.D.5 + 8.D.6 + 8.D.6b (Fase D **frontend**: páginas Next.js `/dashboard/support-inside` cliente comparador + gestión, `/admin/support-inside-plans` índice clicable + `/admin/support-inside-plans/[slug]` editor con 5 secciones card, componente reusable `EditorSectionCard` Design System, isolation ADR-075 frontend `PRODUCT_TYPES_CREATABLE`+filas grises+redirect detalle, AdminSidebar item Support Inside, endpoint público `GET /dashboard/support-inside/plans` para comparador, +2 unit tests, suite full 137/137 unit verde) | 2026-05-01 | _(este sprint)_ |
| 8.D.12 (visibilidad transversal Support Inside): D.12.1 schema `anniversary_day` + migración `sprint8d12_anniversary_day` + cron diario `0 6 * * *` con filtro `anniversary_day=hoy` (ADR-034 §recurrencia drift cerrado) · D.12.2 listener `support-inside-priority` (`conversation.created` → priority_tier) · D.12.3 listener `support-inside-audit` (4 eventos → AuditService) · D.12.4 helper `clientsService.findOne` enriquecido con SI subscription + slots · D.12.5 badge `ClientDetailHeader` admin · D.12.6 badge `ConversationHeader` tier+SLA + helper `support-query.findOne` con subscription single-query · D.12.7 stat `Mi plan SI` en dashboard cliente con CTA si no tiene · D.12.8 modal asignar slot desde `/dashboard/support-inside` + endpoint `eligible-services` · D.12.9 ADR-076 refactor checkout vía evento `service.provisioned` + listener `support-inside-on-service-provisioned` + comparador SI redirige a `/dashboard/billing/checkout?product_pricing_id=` · D.12.10 seed `sample-support-inside.ts` activa Carla con plan Medium + 1 slot + degradación elegante · D.12.11 17 unit tests nuevos (8 priority + 4 audit + 4 service-provisioned + 2 anniversary cron). Suite full **154/154 unit verde** sin regresión. Backend typecheck + lint + build verde. Frontend typecheck + lint (0 errores; 48 warnings DC.6 preexistentes) + build verde. | 2026-05-01 | _(este sprint)_ |
| 8.D.12.fix (auditoría post-test Yasmin 2026-05-01): (a) Bug **slot al propio plan SI**: `eligible-services` filtra server-side `product.type !== 'support_inside'` + `addSlot()` rechaza con 400 + mensaje accionable (defense in depth doble). (b) Bug `Console Error {}` del checkout: `useCheckout.loadProducts/loadProfiles` migrados a `console.warn` + `getErrorMessage(e)` (el helper `api()` lanza objetos plain `{status, message, correlationId}` sin prototype Error que `console.error` serializaba como `{}`). (c) **`applicable_product_types` por plan** (feature mejora canónica): nuevo campo `ProductType[]` en `support_inside_config`, migración `sprint8d12_applicable_product_types` con backfill canónico `['hosting_web','docker_service']`. Backend `addSlot()` rechaza si `service.product.type` ∉ `applicable_product_types` (empty array = sin restricción legacy/Enterprise). `eligible-services` filtra por el array. Editor admin §3 "Slots y capacidades" con multi-select chips (5 tipos sin `support_inside`). Cliente empty state contextual enumera tipos cubiertos. (d) **Clarificación doctrinal DC.16**: `services.credit_balance_eur` reescrita como "buffer técnico de prorrateo, NO sistema de créditos". ADR-029 §"Cuándo revisar" con frontera explícita: si surge demanda wallet/loyalty → ADR-NNN nuevo con `credit_transactions`. +2 unit tests del guard (rechazo type ∉ array · empty array sin restricción). Suite **157/157 unit verde**. Backend lint + build verde. Frontend typecheck + lint (0 errores; 48 warnings DC.6 preexistentes) verde. | 2026-05-01 | _(este sprint)_ |

**Estado DoD final 8.D.12 (al cierre 2026-05-01):**

- ✅ Backend typecheck + lint + build + **157/157 unit tests** (137 previos + 17 D.12 + 3 D.12.fix nuevos: 1 defense-in-depth `support_inside`-self · 2 `applicable_product_types` rechazo+empty).
- ✅ Frontend typecheck + lint (0 errores; **48 warnings DC.6** — los 4 nuevos respecto al 44 base son del mismo patrón fetch+setState ya gobernado por DC.6) + build verde con 3 rutas Next.js detectadas (`/dashboard/support-inside`, `/admin/support-inside-plans`, `/admin/support-inside-plans/[slug]`).
- ✅ **3 migraciones nuevas aplicadas** sin regresión: `sprint8d12_anniversary_day` (campo + CHECK 1..28 + idx + backfill), `sprint8d12_applicable_product_types` (`ProductType[]` + backfill canónico), `sprint8d_cleanup_legacy_support_inside_basic` (D.9.fix previa, sigue limpia).
- ✅ **3 listeners transversales** registrados en `SupportInsideModule.providers`:
  - `SupportInsidePriorityListener` consume `conversation.created` → mapea `priority_tier` → `ConversationPriority`. Compare-and-swap: solo escala si `priority='normal'` (preserva elección manual del agente — EC-T8-47).
  - `SupportInsideAuditListener` consume los 4 eventos canónicos (`subscribed/cancelled/slot_assigned/slot_released`) → `AuditService.logChange()` con `entity_type` distinguiendo subscription vs slot. Cumple R3 (audit inmutable) + portal transparencia cliente.
  - `SupportInsideOnServiceProvisionedListener` consume `service.provisioned` (emitido por `BillingCheckoutService.checkout`) → crea/reactiva `SupportInsideSubscription` cuando `product.type='support_inside'`. Materializa ADR-076 (checkout único por dominio billing).
- ✅ **2 helpers transversales single-query** (sin N+1) que enriquecen respuestas existentes con info SI:
  - `clientsService.findOne` admin → ahora incluye `support_inside_subscription { product, support_inside_config, slots }`.
  - `supportQueryService.findOne` → ahora incluye `client_support_inside { product_slug, product_name, priority_tier, response_sla_hours, channels_active }` enriquecido desde el `User` del owner.
  - `dashboardService.getClientOverview` → ahora incluye `support_inside { product_name, slug, priority_tier, response_sla_hours, slots_included, slots_used }`.
- ✅ **Endpoints nuevos**: `GET /dashboard/support-inside/eligible-services` (modal asignar slot — filtra por `applicable_product_types` server-side).
- ✅ **Cron `maintenance-monthly` doctrinalmente canónico** (ADR-034 §recurrencia drift cerrado): `0 6 * * *` UTC diario filtrando `anniversary_day = EXTRACT(DAY FROM NOW())`. `addSlot()` setea `anniversary_day = LEAST(getUTCDate(), 28)`. Distribuye carga del equipo a lo largo del mes.
- ✅ **3 badges + 1 stat + 1 modal frontend transversales**:
  - Badge `Support Inside {tier}` en `ClientDetailHeader` admin con tooltip SLA + canales, link a `/admin/support-inside-plans/<slug>`.
  - Badge `{plan} · SLA {N}h` en `ConversationHeader` (solo admin) con tooltip canales.
  - Card `Support Inside` en `ClientStats` overview con dual estado activo/CTA y link a `/dashboard/support-inside`.
  - Modal "Asignar slot" en gestión cliente con select de servicios elegibles + select tipo limitado por plan + empty state contextual enumerando tipos cubiertos.
- ✅ **ADR-076 stub creado** (`adr-076-checkout-unico-support-inside-via-evento.md`): formaliza eliminación del modal subscribe + flujo único `/dashboard/billing/checkout` + listener consume `service.provisioned`. ADR-061 + ADR-034 actualizados con notas de revisión 2026-05-01 apuntando a ADR-076.
- ✅ **`applicable_product_types` por plan** (mejora 2026-05-01): editor admin con chip multi-select; cliente solo ve servicios compatibles; backend rechaza con mensaje accionable si tipo incompatible. Doctrina default canónica = `['hosting_web', 'docker_service']`. Empty array = sin restricción (futuro plan Enterprise).
- ✅ **Seed canónico `sample-support-inside.ts`**: activa Carla con plan Medium + 1 slot mantenimiento sobre `Web demo Carla` (hosting Pro). Idempotente, marker `metadata.seeded=true`, salvaguarda `NODE_ENV=production`. Re-seed sin duplicar (verificado en runtime).
- ✅ **11 deudas transversales DC.16..26 documentadas** en `backlog.md` con prioridades (P1/P2/P3) y dependencias entre sí. DC.16 reescrita con marcador "buffer técnico de prorrateo, NO sistema de créditos" tras pregunta Yasmin. ADR-029 §"Cuándo revisar" añade frontera explícita anti wallet de facto.
- ✅ **Decisiones técnicas registradas**:
  - **Listener priority NO pisa elección manual**: compare-and-swap `WHERE priority='normal'` → si el agente cambió priority entre create y listener, count=0 y log debug (EC-T8-47).
  - **Audit listener separa entity_type**: `SupportInsideSubscription` vs `SupportInsideSlot` para que portal cliente filtre por tabs.
  - **Listener service.provisioned con filtro defensivo**: `if (product_type !== 'support_inside') return;` permite coexistencia con futuros listeners hosting/docker (Sprint 11) sin acoplamiento.
  - **`applicable_product_types` empty array = sin restricción**: reservado para plan "Enterprise" futuro (ADR posterior). Default seed canónico = mantenibles reales (`hosting_web`, `docker_service`).
  - **`anniversary_day` capado a 28** (no 31): evita `EXTRACT(DAY)` en febrero quedarse sin disparar slots de día 29-31. Doctrina ADR-034.
  - **Helper `dashboardService.getClientOverview` con `support_inside_subscription` con slots `released_at: null`**: count single-query en lugar de query separada, evita N+1.
  - **Bug `Console Error {}`**: el helper `api()` lanza shape `{status, message, correlationId}` plain (no Error). `console.error()` los serializa como `{}` por falta de prototype. Migrado a `console.warn(prefix, getErrorMessage(e))` en useCheckout. Defecto preexistente, no introducido por D.12.
  - **DC.16 NO es sistema de créditos**: clarificación post-pregunta Yasmin. `services.credit_balance_eur` es buffer técnico interno del flujo de prorrateo (ADR-029), invisible al cliente. Si surge demanda real de wallet/loyalty/refunds visibles, ADR-NNN nuevo con tabla dedicada `credit_transactions`. Frontera documentada en ADR-029 §"Cuándo revisar".
...
- ✅ **Fase D backend cerrada 2026-05-01** (ver bloque "Fase D backend" más abajo).
...
**Estado DoD final Fase D backend (al cierre 2026-05-01):**

- ✅ Backend typecheck + lint + build + **135/135 unit tests** (107 previos + 28 nuevos: 15 `SupportInsideService` + 7 `MaintenanceMonthlyService` + 6 `SupportInsideIsolationGuard`).
- ✅ E2E suite full **117/117 verde** sin regresión (112 previos + 5 nuevos en `support-inside.spec.ts`: subscribe end-to-end + slot + cron mensual + idempotencia P2002 / admin GET 3 planes / admin PATCH Pro Precios / 400 isolation guard / 403 defense in depth cliente).
- ✅ **2 migraciones nuevas**: `sprint8d_support_inside_schema` (3 tablas + 5 enums + relaciones inversas User/Service/Product) + `sprint8d_cleanup_legacy_support_inside_basic` (limpieza versionada del producto huérfano del seed previo, no SQL ad-hoc).
- ✅ **Seed canónico 3 planes** Básico/Medium/Pro con `support_inside_config` + pricing mensual+anual con 15% descuento, idempotente por slug, integrado en `seed.ts` orquestador. Operación canónica de la empresa, NO demo data — se siembra incluso en `NODE_ENV=production`.
- ✅ **1 cola BullMQ scheduled nueva**: `maintenance-monthly` (`0 6 1 * *` UTC) replica patrón canónico Fase C — service testeable + processor delgado + DLQ + endpoint admin trigger manual `POST /api/v1/admin/support-inside/cron/maintenance-monthly`.
- ✅ **Aislamiento ADR-075 §A.2 backend**: `SupportInsideIsolationGuard` aplicado a `AdminProductsController` rechaza POST/PATCH/DELETE sobre `type=support_inside` salvo header interno `X-Aelium-Source: support-inside-admin`. Defense in depth: aunque el frontend filtre, la API protege.
- ✅ **6 endpoints cliente** + **3 endpoints admin** + **1 endpoint admin trigger cron** activos con triple guard (`JwtAuthGuard` + `AdminOnlyGuard` para admin + `PoliciesGuard`).
- ✅ **CASL extendido**: `Subject.SupportInside` ya existía; ampliado `client` con `[Read, List, Update]` para gestionar su propia subscription. `superadmin` (Manage.All wildcard) y `agent_full` (Manage.SupportInside explícito) editan planes desde `/admin/support-inside-plans`.
- ✅ **4 eventos nuevos** (`support_inside.subscribed/cancelled/slot_assigned/slot_released`) emitidos como hooks aspiracionales — sin consumidor todavía (audit / notifications futuras se enganchan sin tocar `SupportInsideService`).
- ✅ Doc canónica completa: ADR-075 + nota revisión ADR-061, products/contract.md §17, jobs-reference.md (cola nueva + tabla resumen), `_events.md` sección `support_inside.*`, current.md §10 cierres registrados.
- ✅ **Decisiones técnicas registradas**:
  - **upgrade() MVP rechaza** con mensaje accionable (ADR-029 prorrateo pendiente sprint dedicado). Workaround: cliente cancela y recontrata.
  - **subscribe() reactiva subscription cancelada** (no crea duplicada — UNIQUE client_id lo exige).
  - **MaintenanceMonthlyService usa cola pública ADR-072** (`assigned_to=null` en task creada) — el agente la auto-asigna desde `/admin/tasks?scope=unassigned`.
  - **SupportInsidePlansAdminService llama directo a Prisma** (no via `ProductsService`) para editar las 3 tablas en una sola transacción atómica y evitar las invariantes PROD-INV-2/3 que no aplican al editor dedicado.
  - **NO hay `POST` ni `DELETE` en `/admin/support-inside/plans`** — un cuarto plan exige migración + seed + ADR específico (cambiar la oferta comercial merece auditoría git, no clic en UI).

**Estado DoD final Fase D frontend (al cierre 2026-05-01):**

- ✅ Backend typecheck + lint + build + **137/137 unit tests** (135 previos + 2 nuevos `listPublicPlans` + ningún test perdido).
- ✅ Frontend typecheck + lint (0 errores; 47 warnings DC.6 preexistentes — los 3 nuevos son del mismo patrón fetch+setState que aún tiene postura DC.6 abierta) + build (3 rutas nuevas estáticas/dinámicas detectadas: `/dashboard/support-inside`, `/admin/support-inside-plans`, `/admin/support-inside-plans/[slug]`).
- ✅ **Endpoint público nuevo**: `GET /api/v1/dashboard/support-inside/plans` (`SupportInsideService.listPublicPlans()`) — devuelve los 3 planes activos con `support_inside_config` + `pricing` mapeados al shape comparador cliente. Se añadió porque `/products?type=support_inside` no incluye `support_inside_config`. Triple guard heredado del controller (`JwtAuthGuard` + `PoliciesGuard` con `Read.SupportInside`).
- ✅ **3 rutas Next.js cliente/admin**:
  - `/dashboard/support-inside` (cliente): comparador 3 cards lado a lado (toggle mensual/anual con −15% badge) si no hay subscription; vista de gestión con slots activos + canales + SLA + acción cancelar (modal confirmación · libera slots cascada) si hay subscription activa. Plan recomendado destacado (`badge_text` o segundo plan por defecto).
  - `/admin/support-inside-plans` (admin índice): tabla vertical 3 filas con columnas Plan / Mensual / Anual + savings% / Slots / Estado / Última edición. NO formato comparador (ADR-075 §B.2). Click → editor. Sin botón "Crear plan" (un 4º plan exige migración + ADR específico).
  - `/admin/support-inside-plans/[slug]` (admin editor): pila vertical de **5 secciones card** (Identidad · Precios · Slots y capacidades · Soporte y canales · Configuración avanzada). Cada card guarda su subset con botón "Guardar cambios" propio (NO auto-save · ADR-075 §B.2). Estado dirty visible (punto warning + banner "Cambios sin guardar" + botón "Descartar"). `beforeunload` warning si hay cualquier sección dirty.
- ✅ **Componente Design System reusable**: `<EditorSectionCard>` en `frontend/app/components/ui/EditorSectionCard/` con CSS module canónico. Patrón documentado en JSDoc: cada sprint añade UNA card nueva al final, NO redistribuye campos entre cards existentes. Pensado para crecimiento Sprint 9.5/12/15F sin redesign (Notificaciones por plan, IA copilot, Métricas operativas, etc.). Exportado desde `components/ui/index.ts` barrel.
- ✅ **Aislamiento ADR-075 frontend**:
  - Constante canónica `PRODUCT_TYPES_CREATABLE` en `app/admin/products/new/constants.ts` excluye `support_inside`. Alias `PRODUCT_TYPES = PRODUCT_TYPES_CREATABLE` mantiene retrocompatibilidad de los imports existentes sin cambiar más callsites. El backend `SupportInsideIsolationGuard` ya bloqueaba la creación con 400 (defense in depth, ya cerrado D.4b).
  - Listado `/admin/products`: filas con `type='support_inside'` se renderizan con `opacity:0.7`, badge "Tier de cuenta" (variant `neutral`), botón único "Gestionar →" enlazando a `/admin/support-inside-plans/<slug>`. `onRowClick` redirige a la página dedicada en lugar de al detalle genérico.
  - Detalle directo `/admin/products/[id]` con `type='support_inside'`: `router.replace('/admin/support-inside-plans/<slug>')` con toast info — el admin no se topa con un 400 del guard backend.
- ✅ **AdminSidebar**: nuevo item "Support Inside" en sección Operaciones, visible para `superadmin` + `agent_full` (vía `Manage.SupportInside`/`Subject.SupportInside` ya en CASL backend D.3+D.4). `SIDEBAR_PERMISSIONS['superadmin']` y `['agent_full']` actualizados con `'SupportInside'`. `ROUTE_PERMISSIONS['/admin/support-inside-plans'] = 'SupportInside'`.
- ✅ **API frontend canónica**: `supportInsideApi` en `lib/api.ts` con tipos exhaustivos `SupportInsidePublicPlan`, `SupportInsideSubscriptionPayload`, `SupportInsideAdminPlanRow`, `SupportInsideAdminPlanDetail`, `SupportInsidePlanPatch`. Métodos cliente (`listPlans`, `getStatus`, `subscribe`, `cancel`, `addSlot`, `releaseSlot`) y admin (`adminList`, `adminGet`, `adminUpdate`).
- ✅ **Decisiones técnicas registradas**:
  - **Comparador `monthly`/`yearly` con toggle**: el cliente conserva la elección durante la sesión sin reload. Anual muestra equivalente mensual + savings% calculado vs mensual×12 (no usa `discount_percentage` directamente — más fiable porque el seed puede dejar el discount sin set).
  - **Cancelación de suscripción detrás de modal de confirmación**: cancelar libera slots cascada y se considera operación destructiva (UX diferenciada).
  - **`pendingPlan` con modal de confirmación antes de subscribe**: el admin/cliente revisa el plan elegido + ciclo + total antes de disparar el checkout. Evita clicks accidentales en el comparador.
  - **`shallowEqual` + `arrayEqualUnordered` por sección** (no JSON.stringify): comparación explícita campo a campo permite manejar el caso `slot_types_allowed` y `channels_active` donde el orden no importa pero el contenido sí.
  - **Patch con sólo deltas**: cada sección envía sólo los campos que cambiaron respecto al snapshot, no el shape completo. Reduce ruido en el endpoint y permite el patrón "guardar una sección a la vez" sin riesgo de pisar otra que el admin no tocó.

---

### ✍ Próxima sesión — orden recomendado

> **Fase D backend 100% cerrada el 2026-05-01** (D.1..D.10 + cleanup migration + ADR-075 + 28 tests unit + 5 E2E).
> **Fase D frontend 100% cerrada el 2026-05-01** (D.5 + D.6 + D.6b + EditorSectionCard DS + isolation ADR-075 frontend + AdminSidebar + endpoint público comparador + 2 unit tests).
> **Sub-fase 8.D.12 100% cerrada el 2026-05-01** — visibilidad transversal completa (D.12.1..D.12.11) + auditoría post-test fix (slot al propio plan SI + Console Error {} + `applicable_product_types` por plan + clarificación DC.16 buffer técnico ≠ sistema de créditos). ADR-076 stub creado, ADR-061 + ADR-034 con notas revisión, 11 deudas DC.16..26 en backlog. Suite **157/157 unit verde** + Backend lint/build verde + Frontend typecheck/lint (0 errores)/build verde. 3 migraciones aplicadas. Carla seedeada activa. La cola activa retoma desde **Fase E** (docs canónicas + retrospectiva).

Mensaje sugerido para arrancar la sesión nueva con contexto fresco: *"Lee `docs/90-meta/development-playbook.md`, `docs/60-roadmap/current.md` §Sprint 8 Fase E. Vamos a cerrar Sprint 8 Fase E: docs canónicas `docs/features/tasks/admin.md`+`agent.md`, `docs/features/support-inside/admin.md`+`client.md`, retrospectiva `docs/60-roadmap/completed/sprint-8-tasks-support-inside.md`, smoke testing manual end-to-end punta a punta, DoD verificado y mover Sprint 8 entero a `completed/`."*
...
3. ~~**Sprint 8 Fase D — Support Inside**~~ ✅ cerrado 2026-05-01 (D.1..D.11 backend + D.5/D.6/D.6b frontend) — schema `support_inside_*` + service + endpoints cliente + admin + 3 páginas dedicadas + cancelación cascada + `MaintenanceMonthlyCron` + seed 3 planes Básico/Medium/Pro.
4. ~~**Sprint 8 Fase D.12 — Visibilidad transversal**~~ ✅ cerrado 2026-05-01 (D.12.1..D.12.11 + fix auditoría post-test). 3 listeners + 3 helpers single-query + 3 badges UI + cron diario doctrinal + ADR-076 + `applicable_product_types`. Suite 157/157 unit. 33% del Sprint 8 completado (de un objetivo Fase E + retrospectiva pendiente).
5. **Sprint 8 Fase E** — docs canónicas: `docs/features/tasks/admin.md` + `agent.md` + `docs/features/support-inside/admin.md` + `client.md` + retrospectiva en `completed/sprint-8-tasks-support-inside.md` + smoke testing manual end-to-end + DoD final + mover Sprint 8 a `completed/`.

#### Atención al arrancar Fase E

- **Crear `docs/features/` si no existe** — la convención canónica `60-roadmap/_sprint-template.md` §DoD lo declara. Si la carpeta no existe, crearla con un `README.md` que explique el patrón "una carpeta por feature × dos archivos por audiencia (admin, agent/client)".
- **Smoke testing punta a punta** (con Carla seedeada activa): login cliente → ver badge SI en overview → asignar slot → liberar slot → cancelar plan → resubscribirse desde comparador (que redirige a checkout canónico) → ver subscription reactivada por listener `service.provisioned`. Login admin → editar plan → cambiar `applicable_product_types` → verificar que cliente lo ve reflejado en eligibles. Verificar badges en `ClientDetailHeader` y `ConversationHeader`.
- **Retrospectiva `completed/sprint-8-tasks-support-inside.md`**: documentar lecciones (drift `anniversary_day` detectado vs. doctrina ADR-034 — auditoría es necesaria), patrones canónicos (EditorSectionCard reutilizable, listener-based transversal, helper single-query con include), deudas registradas DC.16..26.
- **Mover Sprint 8 entero a `completed/`** una vez retrospectiva escrita y `current.md` reducido a la sección Sprint 9.6/11.5 (si hay sprint en curso) o vacío. `current.md` no debe contener sprints cerrados.
