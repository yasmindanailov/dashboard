...
| 15D.II.B0-bis | Research OT&E | Captura shapes reales de `validate-transfer`/`transfer`/`resend-rfa`/`cancel-transfer`/`restore`/`v5-suggest-names` contra OT&E sandbox **antes de codear** (patrón L20). | ⬜ |
| 15D.II.T1 | Transfer transporte | Métodos cliente RC + tipos + `MockResellerClubServer` con FSM simulable (additivo, sin contrato). | ⬜ |
| 15D.II.T2 | Transfer núcleo | `provisionTransferIn` + FSM `pending→awaiting_auth→submitted→{completed\|failed}\|cancelled` + checkout (cobro al completar) + reconcile cron motor + DOM-INV-6. | ⬜ |
