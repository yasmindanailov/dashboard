...
> **Orden de ejecución canónico** (decisión Yasmin 2026-05-09): `E → E.2 → F → G → H → J → I`. La **siguiente fase a abordar tras merge PR #44 es 15C.E.2** — desbloquea contratación operativa real (sin ella ningún producto Enhance es contratable end-to-end). Las fases backend F/G/H pueden encadenarse o paralelizarse según dependencias internas. J cierra el alcance operativo (admin page + dev seed). I es la última fase del sprint (E2E completo + retrospectiva + cierre documental + smoke contra Enhance live).

- 🔄 **15C.E** — Acciones curadas backend (reset_password + view_disk + view_bandwidth + change_package admin + force_resync admin) + audit completo + flag canónico `ServiceAction.adminOnly` ([ADR-077 Amendment A3](../10-decisions/adr-077-contrato-provisioner-plugin-v2.md#amendments)) + 10ª action `list_available_plans` ([ADR-083 Amendment A3](../10-decisions/adr-083-plugin-enhance-cp-specifics.md#amendments)) + enforcement HTTP 403 backend + evento `service.action_admin_only_violation`. **En review** PR [#44](https://github.com/yasmindanailov/dashboard/pull/44) — 9 commits + suite 454/459 + 5 skipped + lint:check + build verde (0.5-1 sesión).
- ⏳ **15C.E.2** ⭐ ⚡ **SIGUIENTE FASE — abordar tras merge PR #44** — Frontend acciones curadas (NUEVA — gap descubierto Fase E review): form admin productos extendido con sub-form dinámico `provisioner_config` via `@rjsf/core` JSON-Schema 7 (patrón heredado Sprint 15A plugin install UI; para `enhance_cp` campo `enhance_plan_id: integer` required) + filter `adminOnly` en `ActionsBar.tsx` con prop `isAdmin` desde AuthContext. **Sin Fase E.2 ningún producto Enhance es contratable operativamente** (bloqueante operativo: producto creado vía UI queda con `provisioner_config: null` → `INVALID_PAYLOAD: enhance_plan_id missing` al provisionar). 1 sesión.
