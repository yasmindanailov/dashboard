...
> **Última actualización:** 2026-05-23 — **🚀 Sprint 15D (ResellerClub + comercio de dominios) EN CURSO.** Fases **15D.A→D ✅** mergeadas (master `ae70010`, PR #108): doctrina (ADRs 077-A10 / 082-A2 / 084 / 081) → fundación+contrato → cliente HTTP RC + mock → plugin core (`provision(register)` + DOM-INV-1 + `domain.registered` vía Outbox). **Próxima: 15D.E** (renovación + DOM-INV-4 + lifecycle de expiración + 3 crons). Detalle por fase en §Sprint 15D abajo.
>
