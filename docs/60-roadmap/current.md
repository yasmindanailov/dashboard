...
| 15D.II.S | Buscador rico | **Congela `suggestDomainNames?()`** (contrato additivo, capability-driven) + `DomainSuggestion`. **suggest-names:** RC client `suggestNames`→`/domains/v5/suggest-names` (la v4 está muerta, A1.5; shapes CONSERVADORES hasta smoke G, A7.4) + mock + plugin (parseo defensivo) + `DomainsService.suggestDomains` (enriquece con precio server-side R5, solo comprables, fail-soft) + `POST /domains/suggest`. **bulk multi-nombre:** refactor de `checkAvailability` (helper per-SLD + `resolveRegistrarPricing` compartido) → `checkAvailabilityBulk(slds)` (dedup + cap fan-out) + `POST /domains/check-availability-bulk`. **Frontend:** `DomainSearch` auto-detecta 1 nombre (resultados + sección *Sugerencias*) vs varios (búsqueda en bloque agrupada); todo al carrito único. IDN → v1.1. Verde: typecheck + lint + **1196 unit** (+7) + boot 4/4. | ✅ |
