...
| 15D.II.T3 | Transfer cierre | Eventos `domain.transfer_initiated/failed` vía Outbox + notifs + zona DNS al completar (capability-routed) + **reintento** (A2.5: `failed`/`cancelled` → reabrir a `pending`). | ⬜ **(siguiente)** |
| 15D.II.R | Restore (RGP) | `restoreDomain`→`domains/restore`; cierra el CTA `recoveryHint='restore'` (DOMAIN_IN_REDEMPTION). | ⬜ |
