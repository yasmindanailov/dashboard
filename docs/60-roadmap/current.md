...
| 15D.II.T2c.3 | Transfer entrada (UX) | **Endpoint REST** `submit_transfer_auth_code`/iniciar transfer → `initiateTransferIn` + **ajuste del checkout** (el `transfer_in` NO crea invoice line al pedir; crea el service `pending` sin cobro). **Decisión UX pendiente:** ¿transfer en el carrito único o flujo dedicado? Acoplado al frontend. | ⬜ **(siguiente)** |
