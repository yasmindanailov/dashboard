...
| 15D.II.T2c | Transfer entrada + billing | **Iniciación síncrona** (`initiateTransferIn`, auth-code en memoria R12 — NUNCA cola/metadata) + acción curada `submit_transfer_auth_code` (re-submit en `awaiting_auth`) + **checkout cobro-al-completar** (el transfer NO factura al pedir; se despacha síncrono) + **factura en `transfer_completed`** (vía `BillingInvoiceService`, ya testeado) + endpoint REST + boot smoke. La rebanada de dinero/entrada (acoplada). | ⬜ **(siguiente)** |
