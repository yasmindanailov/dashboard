...
| 15D.II.T2b | Transfer núcleo (billing/DI) | Orquestador (thread `transferAuthCode`+`dnsTargetHint`) + **checkout cobro-al-completar** (el transfer no factura al pedir → despacha `provision` directo; auth-code en el dispatch payload, R12) + **reconcile cron motor** (avanza `submitted→completed/failed` + activa + `expires_at`) + **factura en `transfer_completed`** + boot smoke. Prerequisito HIGH-3 (`BillingInvoiceService` spec) **ya cerrado** en 15D.E. | ⬜ **(siguiente)** |
