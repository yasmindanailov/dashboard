...
## 🔄 Sprint 15C — Plugin Enhance CP (P2.3) (Fases A+B+C+D+E+E.2+F+G cerradas; H+J+I pendientes)
...
>
> **Última actualización:** 2026-05-09 — **Fases 15C.E + 15C.E.2 + 15C.F + 15C.G mergeadas a master en cadena el mismo día** (4 PRs encadenados sin desincronización). Sprint pasa de 4 a 8 fases cerradas; quedan 3 fases pendientes (H + J + I). Suite total alcanzada: **466/471 unit verde + 5 skipped** (+21 vs base post-Fase D). El bloque DNS-as-capability está completo end-to-end (resolver + endpoints + UI cliente). Pendiente último bloque: reconciliation 3 capas L3 (Fase H) + cierre operativo admin (Fase J) + cierre formal sprint (Fase I).
...
> **Orden de ejecución canónico** (decisión Yasmin 2026-05-09): `E → E.2 → F → G → H → J → I`. **Siguiente fase a abordar es 15C.H** — pieza ligera (0.5 sesión) que cierra el bloque reconciliation 3 capas (L3 cron, las capas L1 cache + L2 reads on-demand ya están cableadas desde Fase D + Fase G). J cierra el alcance operativo (admin page + dev seed). I es la última fase del sprint (E2E completo + retrospectiva + cierre documental + smoke contra Enhance live).

- ⏳ **15C.H** ⚡ **SIGUIENTE FASE** — **Reconciliation L3 cron + listener audit + notificación threshold** (0.5 sesión). Materializa [ADR-083 §6 decisión 24](../10-decisions/adr-083-plugin-enhance-cp-specifics.md). Plan canónico expandido en [dossier §7 fila H](./sprint-15c-enhance-cp-dossier.md#7-estimación-esfuerzo-sprint-15c--11-fases) — el próximo agente debe leerlo antes de codear. Resumen: cron BullMQ scheduled 6h `reconcile-enhance-services` que itera services `provisioner_slug='enhance_cp' AND status IN ('active','suspended')` invocando `plugin.getStatus(service)` y, según resultado, emite `service.reconciled_external_change` con `change_type ∈ {'subscription_missing', 'status_divergence', 'plan_divergence'}`. Listener nuevo `AuditOnServiceReconciledExternalChangeListener` persiste en `audit_change_log` con flag GDPR según `change_type`. Listener segundo `NotificationsOnReconciliationThresholdExceededListener` cuenta divergencias / día contra setting `provisioning.enhance_cp.reconciliation_alert_threshold` (ya seedeado Fase D, default `5`). DH-INV-6 aplica: status_divergence → Aelium adopta Enhance state; plan_divergence → NO auto-corrige (billing implication, requiere admin).
- ⏳ **15C.J** ⭐ — **Cierre real operativo (NUEVA — gap descubierto Fase E review)**: página admin `/admin/services/[id]` SC nativo paralelo al detalle cliente con 3 botones admin operables (`change_package` + `force_resync` + `list_available_plans`) + modal `change_package` (invoca primero `list_available_plans` para dropdown + luego `change_package` con planId elegido) + plugin install seed condicional `NODE_ENV !== 'production'` para DX QA/staging (1 sesión).
- ⏳ **15C.I** — **Cierre formal del sprint** — E2E completo flujo Enhance (producto admin con `provisioner_config.enhance_plan_id` → cliente checkout → orchestrator `provision()` 6-step contra mock → `service.activated` → frontend cliente render N botones filtrados por `adminOnly` → click cliente `view_disk_usage` → 200 + métricas; click cliente `change_package` → 403 + audit `service.action_admin_only_violation`; click admin → 200 + plan changed via `list_available_plans` dropdown; cliente abre `/dashboard/services/[id]/dns` y crea record A apex; admin abre SSO impersonation → cliente lo ve en `/dashboard/transparency`) + smoke manual contra Enhance live (1-2h Yasmin) + cierre documental (`docs/features/provisioning/admin-plugins-enhance.md` + retrospectiva `completed/sprint-15c-plugin-enhance-cp.md` + `_events.md` + `_matrix.md`) + i18n strings finales (1-1.5 sesión).
