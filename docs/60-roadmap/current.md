...
> **Última actualización:** 2026-05-02 — Sprint 11 Fases 11.A (ADR-077) + 11.B (orquestador + chasis canónico) mergeadas en master. Sprint 11 sigue 🟡 WIP — cola Fases 11.C (plugins triviales) → 11.D (frontend) → 11.E (cierre documental + retro).
...
| **11.D** | **Frontend `/dashboard/services/[id]` (Server Component) + endpoints REST**:<br>· Endpoints REST cliente (4) + admin (3) en `provisioning.controller.ts` y `admin/provisioning.controller.ts`.<br>· DTOs con class-validator + CASL `Subject.Service` refinado.<br>· Página `/dashboard/services` (listado cliente — Server Component).<br>· Página `/dashboard/services/[id]` (detalle — Server Component que lee `getServiceInfo` server-side, condicionado por `capabilities` flags).<br>· Componentes DS reusables en `_shared/services/`: `<ServiceHeader>`, `<MetricsBar>`, `<ActionsBar>`, `<SsoButton>`, `<AuditLogFeed>`.<br>· Página `/admin/services` (listado admin con filtros).<br>· Slot UI placeholder "Solicitar desarrollo personalizado" (Sprint 22 Projects lo habilitará).<br>· Tests E2E del flujo cliente (ver detalle servicio + asignar slot SI desde el detalle del servicio — cierra DC.17).<br>· **Doctrina anti-DC.6**: Server Components nativos, NO `useEffect+fetch+setState`. | ⬜ |
...
>
> *"Lee `docs/90-meta/development-playbook.md`, `docs/10-decisions/adr-077-contrato-provisioner-plugin-v2.md`, `docs/20-modules/provisioning/contract.md`, `docs/60-roadmap/current.md` §Sprint 11. Vamos con Sprint 11 Fase 11.C — plugins triviales `internal` + `manual` + listener `provisioning-on-task-completed` + tests E2E. Crea rama `sprint11-fase-c-plugins-triviales` desde master."*

#### Atención al arrancar Fase 11.C

- **Verificar primero que master está sincronizado** y que `pnpm seed` re-aplica el setting `provisioning.service_info_ttl_seconds=60`.
- **Plugins viven en `backend/src/plugins/provisioners/<slug>/`** — directorio NO existe todavía, crearlo con los 2 plugins.
- **Wireado al `ProvisioningModule`**: añadir como providers con `{ provide: PROVISIONER_PLUGINS, useExisting: InternalProvisionerPlugin, multi: true }` (mismo patrón con `ManualProvisionerPlugin`). Reemplazar el `useValue: []` placeholder.
- **Listener `provisioning-on-task-completed`** filtra por `plugin.capabilities.completes_via_task=true` (NO por `task.type === 'support_setup'` hardcoded — diseño abierto a Sprint 22 reutilizando el mismo listener para `project_task` con plugin futuro `project`).
- **EC-P11-07 crítico**: el bridge ticket↔task Sprint 8 también escucha `task.completed`. Asegurar que ambos listeners filtran de forma mutuamente excluyente: bridge por `task.conversation_id != null`, provisioning por `task.service_id != null && plugin.capabilities.completes_via_task`. Test que verifica race no posible.
- **Tests E2E reales** son el bloque de mayor riesgo (~60% del tiempo de la fase): seed productos `provisioner='internal'` + `provisioner='manual'`, login Carla, checkout vía `/dashboard/billing/checkout`, esperar listener async, assert estado.
- **Hito histórico Fase 11.C**: tras esta fase, el listener Sprint 8 D.12.9 (`SupportInsideOnServiceProvisionedListener`) se valida en flujo real con orquestador + plugin `internal` coordinando. Hasta hoy solo está validado en tests unit.
- **NO añadir** plugins reales (cPanel, Docker, ResellerClub) — esos son Sprint 15A/C/D/E. Solo `internal` y `manual` triviales.
- **NO añadir** `services.status='project_development'` — Sprint 22.
