...
| 15D.II.R | Restore (RGP) | `restoreDomain`→`domains/restore`; cierra el CTA `recoveryHint='restore'` (DOMAIN_IN_REDEMPTION). | ⬜ **(siguiente)** |
| 15D.II.S | Buscador rico | `suggest-names v5` + bulk multi-nombre (IDN opcional → v1.1). Congela firma `suggestDomainNames?()`. | ⬜ |
