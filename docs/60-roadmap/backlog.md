...
| **P1.1.5** | **Sprint 9.5 — UX admin de notifications + cabos sueltos Sprint 9** — 7 items diferidos: (a) endpoints `/notifications/unread`, `/:id/read`, `/read-all`; (b) panel admin `/admin/notifications/templates` (GET/PATCH/preview); (c) `NotificationBell` en Topbar cliente con dropdown; (d) cron `cleanupReadNotifications` (`EVERY_DAY_AT_2AM`); (e) seed 4 settings `notifications.*`; (f) listener `notifications-system-error.listener` + plantilla `system.error`; (g) test E2E `notifications.spec.ts` específico de campana. **Plus**: DC.10 cobertura E2E Client en `audit-portal.spec.ts`. **No bloquea Sprint 14 Deploy** — el seed inicial cubre producción. | 1 sesión densa | P1.1 cerrado |
...
| **DC.9** | **Contracts canónicos `audit/`, `notifications/`, `error-log/` con stubs mínimos hoy** — los 3 módulos tienen `contract.md` con secciones 1–5 + referencias a ADRs canónicos (Sprint 9 cierre auditoría). Las secciones 6–12 (CASL completo, eventos consumidos detallados, DTOs, edge cases, métricas, deuda, etc.) quedan pendientes de redacción profunda. **Cuándo abordar**: Sprint 9.5 (1 sesión) o oportunista al tocar el módulo. | Sprint 9.5 (recomendado) |
| **DC.10** | **Cobertura E2E `audit-portal.spec.ts` para path Client/User** — el spec actual cubre solo `Invoice` (que tiene `user_id` directo). El fix `bff4fec` añadió path para `Client`/`User` (donde `id` ES el owner) pero ningún test ejercita ese código. **Cuándo abordar**: Sprint 9.5 (1 spec adicional ~30 líneas). | Sprint 9.5 |
