...
## A.11.9. Handoff Fase F.3 — en curso (estado 2026-05-12, branch `sprint15c-ii-fase-f3-audit-timeline`)

> **Audiencia**: el agente IA que continúe Fase F.3. **Estado 2026-05-12 (sesión 2)**: GAP-M + GAP-N + G8 + G4/G5 + CTA reconcile **+ B (B.1 cooldown del force-refresh + B.2 catálogo ADR-080)** todo en la rama (commits abajo). Close-out **hecho salvo PR/merge**: `pnpm ci:check:full` verde (49 suites, 664+5), boot real verificado, ADR-083 Amendment A7 + ADR-077 Amendment A6 + ADR-080 Amendment C escritas. **Falta solo**: commitear el doc-sync + push + PR (bypass §6 si CI billing-bloqueada) + squash-merge + post-merge sync (`current.md`/`backlog.md`/`MEMORY.md`/`project-state.md` + fila F.3 §A.11.1 → ✅ mergeada). NO re-litigar las decisiones de abajo — ya están tomadas y materializadas. Después de F.3 viene **F.4** (SSL status + alertas de cuota — decidido con Yasmin 2026-05-12) y luego **G** (cierre Sprint 15C.II).
...
| _(pendiente)_ | **doc-sync close-out** | ADR-083 Amendment A7 (registro comprensivo de toda F.3 — GAP-M/N + G4/G5/G8 + CTA + B.1 + B.2 + backlog + validación) + ADR-077 Amendment A6 (`testConnection?()` + `ProvisionerPluginError.module?` — superficie de contrato) + ADR-080 Amendment C (`PluginManifest.serviceInfoCacheTtlSeconds?` — superficie de manifest, por el patrón B.5 de ADR-080) + este §A.11. Va en el PR (no es post-merge — los post-merge son `current.md`/`backlog.md`/`MEMORY.md`/`project-state.md` + flip de la fila §A.11.1). |
...
### A.11.9.2. Pendiente de F.3 (lo que falta)

✅ **HECHO (sesión 2, 2026-05-12)** — item B completo (`5640a8e` B.1 cooldown + `0532012` B.2 catálogo ADR-080), `pnpm ci:check` + `pnpm ci:check:full` verdes, boot real verificado, ADR-083 Amendment A7 + ADR-077 Amendment A6 + ADR-080 Amendment C escritas, este §A.11 actualizado. **Falta solo el cierre outward-facing**:

1. **Commitear el doc-sync del close-out** (ADRs A7/A6/C + dossier §A.11) — `docs(sprint-15c-ii): Fase F.3 — doc-sync close-out`.
2. **PR** (rama `sprint15c-ii-fase-f3-audit-timeline` → master) + si la CI de GitHub sigue bloqueada por billing (firma §A.9.10: jobs Backend/Frontend fallan en 3-11s, `log not found`), **bypass policy §6** con comentario formal en el PR (las 3 condiciones: motivo externo + `ci:check:full` verde + boot real verificado + doc) → **squash-merge** + `--delete-branch`.
3. **Post-merge doc-sync** (patrón #61/#64/#66, su propio PR): `current.md` (Fase F cerrada — F.1+F.2+F.3 en master) + `backlog.md` (`DC.NEW-15CII-RECONCILE-SINGLE`) + `MEMORY.md`/`project-state.md` + flip de la fila F.3 en §A.11.1 → ✅ mergeada con el hash del squash.
