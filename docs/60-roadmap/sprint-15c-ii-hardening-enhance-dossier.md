...
| **F.4** | **Robustez del ciclo de vida (suspend / unsuspend / cancel) end-to-end** — reconciliación del status administrativo en `getInfoForUser` (`services.status` autoritativo para el lifecycle *administrativo*, distinto de DH-INV-6 que aplica al estado *operacional*) + flag `provider_state_desync` en el summary + banner de suspensión para el cliente con CTA por motivo + aviso de desync en la UI admin + **`DC.44`** billing-suspend-unify (`ServiceLifecycleWorker.autoSuspendServices` → `suspendAsAdmin` + auto-reactivación al pagar). Cierra el agujero descubierto en testing (servicio suspendido con la BD `suspended` pero el proveedor `isSuspended:false` → banner "Servicio suspendido" sin botón "Reanudar"; y el desync inverso → `409` en un botón visible). Detalle §A.11.10.1. | ⏳ **PRÓXIMA** — rama `sprint15c-ii-fase-f4-lifecycle-robustness` desde master post post-merge-sync de F.3. |
| **F.5** | **SSL/TLS status read-only + alertas de cuota** — `EnhanceApiClient` lee el estado del certificado que Enhance gestiona; `getServiceInfo` gana `ssl?` (additivo opcional, igual que `metrics`/`recoveryHint` — ADR-077 amendment) + card en el detalle cliente/admin (`_shared/services/`, prop `isAdmin`); alertas de cuota: setting `provisioning.enhance_cp.quota_alert_threshold_pct` + aviso visual en `MetricsBar` al cruzarlo + notificación al cliente al cruzar el umbral (listener + plantilla seedeada — sin spam, persiste la última notificación enviada). Read-only — Aelium NO gestiona el cert (DH-INV-6). Detalle §A.11.10.2. | ⏳ — rama `sprint15c-ii-fase-f5-ssl-quota`. ADR-077 amendment (`ServiceInfo.ssl?`) + ADR-083 amendment (probe SSL de Enhance — path orchd + campos). |
| **F.6** | **Reconciliación per-servicio (`DC.45`)** — `ProvisionerPlugin` gana método opcional `reconcileOne?(service)` (ADR-077 amendment, mismo patrón que A6 `testConnection?()`) + `ReconcileRegistryService.reconcileOne(slug, service)` + `ProvisioningService.reconcileServiceAsAdmin` + endpoint `POST /admin/services/:id/reconcile` + Enhance implementa `reconcileOne` + **wire del CTA**: el botón "Reconciliar contra el proveedor" del `AdminDriftBanner` (cuando `recoveryHint==='reconcile'`) y cada fila de drift del `<PluginOperationalOverview>` (F.2) llaman al endpoint single-shot in-place — cierra el cabo de F.3 (hoy el CTA linka a la página de settings = reconcile-all). Detalle §A.11.10.3. | ⏳ — rama `sprint15c-ii-fase-f6-reconcile-single`. ADR-077 amendment (`reconcileOne?()`) + posible ADR-083 amendment (specifics del `reconcileOne` de Enhance). |
| **F.7** | **UX del detalle de servicio a estándar alto (cliente + admin)** — deep-links curados al panel del proveedor (gestionar email / bases de datos / archivos como atajos SSO, no solo "abrir panel" genérico) + mini-badge de salud del proveedor en `/admin/services/[id]` (derivado del `CircuitBreakerRegistry`, link al `<PluginOperationalOverview>` completo) + notas de servicio admin (tabla nueva `service_notes` append-only + endpoints `GET/POST /admin/services/:id/notes` + UI) + reenviar notificación al cliente + cross-link a billing en la página del servicio ("próxima renovación: X · €Y" → link a la factura). Detalle §A.11.10.4. | ⏳ — rama `sprint15c-ii-fase-f7-service-page-ux`. Migración `service_notes`. Valoración pre-código: ¿deep-links = nuevas inline actions plugin-internas o `getSsoUrl` con parámetro `section`? (ADR-083 amendment si se modela como nueva capability). |
| **G** | **Cierre Sprint 15C.II** — tests críticos faltantes (las 8 áreas del audit técnico Fase A — los que sigan sin cubrir tras F.1-F.7) + E2E spec extension cubriendo Fases E + F.1-F.7 (label `ready-for-e2e` en el PR) + smoke real Yasmin (contra mock + Enhance live si aplica) + retrospectiva (lecciones heredables L19+...) + mover el dossier a `docs/60-roadmap/completed/sprint-15c-ii-hardening-enhance.md` (header retrospectiva + dossier original como anexo de trazabilidad — patrón Sprint 15C) + `current.md`/`backlog.md`/`MEMORY.md`/`project-state.md` (15C.II ✅ CERRADO, **Sprint 15D RC DESBLOQUEADO**). DoD del sprint: todos los DoD de fase ✓ + `pnpm ci:check:full` + `pnpm ci:e2e` verdes + smoke OK. Detalle §A.11.10.5. | ⏳ — última fase. |
...
## A.11.10. Plan de continuación — Fases F.4 → G (re-plan 2026-05-12)
...
>
> **Orden y agrupación** (por prioridad = riesgo descendente): **F.4** robustez del ciclo de vida (arregla un estado roto **actual** — máxima prioridad; absorbe `DC.44`) → **F.5** SSL status + alertas de cuota (gap de feature real; era la "F.4" del plan previo) → **F.6** reconcile per-servicio (`DC.45` — toca el contrato `ProvisionerPlugin`; cierra el cabo del CTA de F.3) → **F.7** UX del detalle de servicio a estándar alto (conveniencias — menor prioridad) → **G** cierre del sprint. **1 rama por fase** (patrón heredado). Cada fase materializa sus ADR amendments **dentro de la fase** (patrón desde Fase E — no hay fase doc-only separada), y arranca con una **valoración pre-código** que coteja el apuntado contra los ADRs frozen relevantes (L18). Cada fase tiene su PR; si la CI de GitHub sigue billing-bloqueada, bypass policy §6 (3 condiciones + doc formal) + post-merge doc-sync PR (patrón #61/#64/#66).

### A.11.10.1. Fase F.4 — Robustez del ciclo de vida (suspend / unsuspend / cancel) end-to-end

**Tema:** el estado de suspensión/cancelación es consistente y recuperable a través de BD ↔ proveedor ↔ cliente ↔ admin ↔ billing — sin estados rotos sin salida.
...
- **F.4.4 — `DC.44` / billing-suspend-unify.** `ServiceLifecycleWorker.autoSuspendServices` (cron impago) → llama a `ProvisioningService.suspendAsAdmin(serviceId, { reason: 'overdue_payment' }, <actor sistema>, { notify_client: true })` en vez de hacer su propio `prisma.service.update` + emitir la forma reducida `{service_id, invoice_id, reason: 'payment_exhausted'}`. Convención "actor sistema": `actor_user_id: null` + `metadata.actor: 'system:billing-overdue-cron'` en el audit. Decisión a tomar (valoración pre-código): ¿`suspendAsAdmin` permite suspender plugins `supports_suspend=false` (`internal`/`manual`) para el caso impago (parámetro `allowUnsupported: true`), o el worker hace su propio `prisma.update` para esos y solo delega a `suspendAsAdmin` los que tienen la capability? **Auto-reactivación al pagar**: nuevo listener (o ampliar `billing-on-invoice-paid`) → si el service estaba `suspended` por impago → `ProvisioningService.unsuspendAsAdmin(serviceId, <actor sistema>)`. (Evaluar también unificar `service.resumed` [flujo pause] ↔ `service.unsuspended` [flujo suspend].) ⚠ Toca el módulo billing (`ServiceLifecycleWorker`, `billing-on-invoice-paid`) — scope expansion **justificada** porque es el fix de raíz del agujero de robustez (no un nice-to-have): tras F.4.4 la BD y el proveedor no divergen para el caso impago, y el desync (F.4.1) queda como red de seguridad para los demás casos.
- **ADR amendments / doc-sync:** ninguno del contrato `ProvisionerPlugin` (el `provider_state_desync` es summary). Doc: nota en ADR-082 §DH-INV-6 (y/o ADR-070) sobre lifecycle administrativo vs operacional; actualizar `DC.44` en `backlog.md` (→ "materializado en F.4"); dossier §A.11.
- **DoD F.4:** status reconciliado verificado en **ambas** direcciones de desync; banner cliente (F.4.2) + aviso admin (F.4.3); `autoSuspendServices` migrado a `suspendAsAdmin`; auto-reactivación al pagar; tests (unit del override en `getInfoForUser` + `provider_state_desync`; listener de auto-reactivación; `autoSuspendServices` migrado); `pnpm ci:check:full` + boot real; PR (+ bypass §6 si aplica) + post-merge sync.
- **Valoración pre-código (L18):** (1) `provider_state_desync` en el summary vs campo nuevo de `ServiceInfo` → recomendado: summary (no es contrato plugin). (2) `suspendAsAdmin` con `allowUnsupported` vs worker hace update propio para `internal`/`manual`. (3) ¿el aviso de desync admin ofrece una acción "re-aplicar en el proveedor" además de informar? (4) cotejar la doctrina "lifecycle administrativo vs operacional" contra ADR-082 DH-INV-6 + ADR-070 (¿amendment o nota inline?).

### A.11.10.2. Fase F.5 — SSL/TLS status read-only + alertas de cuota

**Tema:** visibilidad proactiva del estado del proveedor — cliente y admin ven el estado del certificado y reciben aviso antes de quedarse sin recursos.

- **F.5.1 — SSL/TLS status read-only.** `EnhanceApiClient` gana un método para leer el estado del certificado de la subscription/website (verificar el path en el OAS de orchd v12.21.3); `getServiceInfo` gana un campo **opcional** `ssl?: { status: 'valid' | 'expiring_soon' | 'expired' | 'none'; expiresAt?: string /* ISO */; autoRenew?: boolean; issuer?: string }` (additivo opcional, mismo patrón que `metrics` / `recoveryHint` — **ADR-077 amendment**); card en el detalle de servicio cliente y admin (`frontend/app/_shared/services/SslStatusCard.tsx` + prop `isAdmin` — L16, NO duplicación) con badge verde/ámbar/rojo + "expira en X días" + "renovación automática: sí/no". **Read-only** — Aelium NO gestiona el cert (DH-INV-6); para renovar/cambiar → SSO al panel. La presencia del campo `ssl` es la señal de capability (si ausente, no card — mismo patrón que `metrics`; no se añade un flag nuevo a `PluginCapabilities`).
- **F.5.2 — Alertas de cuota.** Setting `provisioning.enhance_cp.quota_alert_threshold_pct` (default 85, configurable en `/admin/settings/plugins/enhance-cp`). (a) Aviso visual en `MetricsBar` cuando un recurso (disco / ancho de banda) cruza el umbral — la barra se pone ámbar/roja + texto "Estás al X% de tu cuota de disco". (b) Notificación al cliente cuando lo cruza: el reconcile L3 (o un cron dedicado) detecta el cruce — necesita persistir "última notificación enviada por recurso" para no spamear (campo en `services.metadata` o tabla `service_quota_alerts`) → emite `service.quota_threshold_crossed` → listener `notifications-on-service-quota-threshold` → email + campana + plantilla seedeada `service.quota_threshold_crossed`. Heredable a cualquier plugin con `has_metrics`.
- **ADR amendments:** ADR-077 amendment (`ServiceInfo.ssl?` campo opcional) + ADR-083 amendment (specifics del probe SSL de Enhance — path orchd, mapeo de campos, threshold de "expiring soon" p.ej. ≤14 días).
- **DoD F.5:** card SSL cliente + admin (badge + expiry + autoRenew); alertas de cuota (visual en `MetricsBar` + notificación sin spam, con persistencia de "última enviada"); tests; `pnpm ci:check:full` + boot; PR + post-merge sync.
- **Valoración pre-código:** (1) ¿el threshold de "expiring_soon" es fijo (≤14d) o configurable? (2) ¿la persistencia de "última notificación de cuota" va en `services.metadata` o en una tabla dedicada `service_quota_alerts`? (3) ¿el aviso visual de cuota en `MetricsBar` aplica solo a recursos con cuota dura (disco) o también a ancho de banda (que es mensual y se "resetea")?

### A.11.10.3. Fase F.6 — Reconciliación per-servicio (`DC.45`) + cierre del cabo del CTA reconcile

**Tema:** la reconciliación contra el proveedor es granular — el admin puede reconciliar un servicio concreto sin disparar la pasada completa del cron L3.

- **F.6.1 — ADR-077 Amendment:** `ProvisionerPlugin` gana método **opcional** `reconcileOne?(service: ServiceWithRelations): Promise<ReconcileResult>` — análogo al executor del `ReconcileRegistryService` pero para un único servicio (el plugin re-lee el ground truth del proveedor para ese servicio y devuelve los cambios aplicados; DH-INV-6 — actualiza Aelium, no al revés). Additivo opcional, NO bumpea `contractVersion` (mismo patrón que A6 `testConnection?()`); contract test: si el plugin declara `supports_reconciliation: true` y quiere reconcile per-servicio, `reconcileOne` es función (o documentar que es estrictamente opcional).
- **F.6.2 — Backend:** `ReconcileRegistryService.reconcileOne(slug, service)` delega a `plugin.reconcileOne?.(service)` (400 si el plugin no lo implementa). `ProvisioningService.reconcileServiceAsAdmin(serviceId, actorUserId)` (carga el service, resuelve el plugin, llama `reconcileRegistry.reconcileOne`, invalida cache `service_info`, audit `reconciled_single` con el actor real, emite evento `service.reconciled_single`). Endpoint `POST /admin/services/:id/reconcile` (admin, `@CheckPolicies(Update Service)`).
- **F.6.3 — Enhance implementa `reconcileOne(service)`:** extrae las refs, re-lee `getSubscription`, compara contra `services.metadata` / `product.provisioner_config`, aplica los mismos checks que el cron L3 pero para un service (`subscription_missing` / `status_divergence` / `plan_divergence` → actualiza la metadata local / status según corresponda), devuelve `ReconcileResult`.
...
- **DoD F.6:** contrato extendido + Enhance implementa + endpoint + CTA wired (banner + overview); tests (contract invariant, `reconcileServiceAsAdmin`, Enhance `reconcileOne`); `pnpm ci:check:full` + boot; PR + post-merge sync.
- **Valoración pre-código:** (1) ¿`reconcileOne` es estrictamente opcional o obligatorio si `supports_reconciliation: true`? (2) ¿`reconcileServiceAsAdmin` debe permitir reconciliar un service `cancelled`/`terminated`? (no — shortcircuit terminal). (3) nombre del evento (`service.reconciled_single` vs reusar `service.reconciled_external_change` con un marcador).

### A.11.10.4. Fase F.7 — UX del detalle de servicio a estándar alto (cliente + admin)

**Tema:** la página `/services/[id]` (cliente y admin) tiene la riqueza y las conveniencias operativas de un panel reseller profesional.

- **F.7.1 — Deep-links curados al panel del proveedor.** En vez del único botón "Abrir panel del proveedor", un grupo de atajos curados a las secciones más usadas — "Gestionar email", "Gestionar bases de datos", "Administrador de archivos", "Logs del sitio" (las que orchd exponga vía SSO). Materialización (decidir pre-código): (a) nuevas inline actions plugin-internas (`sso_email`, `sso_databases`, `sso_files`...) que internamente generan la URL SSO apuntando a la sección — slugs plugin-internos, NO contrato externo (L14), no necesita ADR; o (b) `getSsoUrl` gana un parámetro `section?: string` y el frontend pide la URL de la sección — esto SÍ es cambio de contrato (ADR-077 amendment) y/o ADR-083 amendment. Cliente + admin (`_shared/`). Si una sección no existe para un plugin/instancia → no se muestra (capability-driven, ADR-070 — cero `if (provisioner === 'X')`).
- **F.7.2 — Mini-badge de salud del proveedor en `/admin/services/[id]`.** Indicador pequeño "Proveedor: operativo / degradado / caído" derivado del `CircuitBreakerRegistry.getState()` para el plugin de este servicio (etiquetado "estado en esta instancia" — el breaker es in-process) + link a `/admin/settings/plugins/[slug]` (el `<PluginOperationalOverview>` completo). Da contexto al admin cuando `getServiceInfo` devuelve `unknown`/fallback.
- **F.7.3 — Notas de servicio (admin).** Migración: tabla nueva `service_notes` `(id uuid, service_id uuid FK, author_user_id uuid FK, body text, created_at)` **append-only** (NO un campo mutable en `services` — append-only es coherente con el espíritu del audit log R3: el admin añade notas, no las edita/borra) + endpoints `GET /admin/services/:id/notes` (paginado) + `POST /admin/services/:id/notes` (audit del POST) + UI en `/admin/services/[id]` (lista cronológica + textarea para añadir). **NO visible al cliente.**
- **F.7.4 — Reenviar notificación al cliente.** En `/admin/services/[id]`, botón "Reenviar notificación al cliente" → modal con selector de qué notificación reenviar (suspensión / cancelación / reactivación / ...) → endpoint `POST /admin/services/:id/notifications/resend` con `{ template_key }` → re-renderiza la plantilla con el contexto del service y la envía + audit. Reusa el historial del módulo `notifications`.
- **F.7.5 — Cross-link a billing en la página del servicio** (cliente + admin): leyendo del módulo billing (la subscription/invoice del service), mostrar "Próxima renovación: X · €Y · [Ver factura]" (read-only en la página del servicio; el link lleva a `/dashboard/billing/[id]` o `/admin/...`).
- **ADR amendments:** posible ADR-083 amendment (deep-links SSO a secciones, si se modela como nueva capability — opción (b) de F.7.1). Migración: `service_notes`.
- **DoD F.7:** deep-links curados (cliente + admin, capability-driven); mini-badge de salud; notas de servicio (append-only); reenviar notificación; cross-link billing; tests; `pnpm ci:check:full` + boot; PR + post-merge sync.
- **Valoración pre-código:** (1) deep-links: ¿inline actions plugin-internas (sin ADR) o `getSsoUrl({section})` (ADR amendment)? — recomendado: la que menos toque el contrato si orchd lo permite. (2) notas de servicio: ¿tabla dedicada `service_notes` o `audit_change_log` con `action='admin_note'`? — recomendado: tabla dedicada (notas ≠ cambios). (3) reenviar notificación: ¿selector libre de `template_key` o whitelist de las plantillas de service-lifecycle? — whitelist.

### A.11.10.5. Fase G — Cierre Sprint 15C.II

**Tema:** DoD del sprint completo — tests críticos, E2E, smoke real, retrospectiva, desbloqueo de Sprint 15D RC.

- **G.1 — Tests críticos faltantes.** Las 8 áreas del audit técnico Fase A (§A.2 coverage gaps) que sigan sin cubrir tras F.1-F.7 — típicamente: advisory lock concurrente real Postgres (`EnhanceCustomersService` 3-step), CircuitBreaker E2E con Enhance, SSO impersonation E2E (audit GDPR), AdminOnly enforcement E2E con bypass curl real, encryption key rotation, DNS edge cases (TTL bounds, conflicto CNAME), `change_package` metadata rollback en fallo, threshold race condition en reconciliación concurrente. Más: tests de F.4 (reconciliación de status, ambas direcciones), F.5 (SSL card states, alertas de cuota sin spam), F.6 (`reconcileOne`), F.7.
- **G.2 — E2E spec extension.** Ampliar el spec E2E (`sprint-15c-enhance-flow.spec.ts` o el que corresponda) cubriendo los flujos de Fases E + F.1-F.7: suspender→reanudar (incl. el desync), banner de suspensión cliente, SSL card, deep-links curados, notas de servicio, reconcile per-servicio. Label `ready-for-e2e` en el PR.
...
- **DoD del sprint 15C.II:** todos los DoD de fase (A→F.7) ✓ + `pnpm ci:check:full` verde + `pnpm ci:e2e` verde + smoke real OK + retrospectiva escrita + dossier en `completed/` + Sprint 15D RC desbloqueado.

### A.11.11. Sesiones origen del re-plan F.4→G

- 2026-05-12 (sesión 2 — post-merge de F.3 #67/#68; el testing de F.1 destapó el agujero de robustez del status de suspensión; decisión Yasmin: traer los apuntados de backlog [`DC.44`, `DC.45`, deep-links, notas, mini-badge, reenviar notif, cross-link billing] al sprint; auditoría "qué falta a estándar alto en el plugin Enhance + módulo de servicios cliente/admin" → este §A.11.10 con las 5 fases F.4→G ordenadas por prioridad).
