...
> ✅ **SPRINT CERRADO 2026-05-21.** Retrospectiva ejecutiva + métricas + lecciones (L13–L23) + DoD en [`completed/sprint-15c-ii-hardening-enhance.md`](completed/sprint-15c-ii-hardening-enhance.md). Este dossier se preserva **in-situ** como **anexo de trazabilidad** (plan + cierre commit-by-commit por fase; referenciado por 11 documentos). **Sprint 15D ResellerClub DESBLOQUEADO.**
...
**Resultados:** integración 4 suites · 8 passed (`pnpm --dir backend test:e2e`, requiere docker dev arriba) + unit **58 suites · 852 passed + 6 skipped** (+3 suites / +54 tests vs baseline 798+6, **cero regresión**) + `typecheck` + `lint:check` verdes. **Cero cambios de contrato ADR-077** (el hardening reusa `PROVIDER_INTERNAL_ERROR`). **Mergeada a master**: PR [#97](https://github.com/yasmindanailov/dashboard/pull/97) squash `983adca` (bypass §6 16ª, 2026-05-21) + post-merge doc-sync `current.md` (`d4234cb`). Restan áreas **4 / 6 / 7e** en **G.2**.

#### A.11.10.10.2. Plan G.2 — extensión E2E — FROZEN 2026-05-21

> **Freeze doc-only (G.2.0).** Congela el alcance de G.2 antes de codear (decisión Yasmin: "freeze + slicing, luego implemento"). G.2 cierra las 3 áreas §A.2 que son inherentemente de flujo (4/6/7e) + formaliza en spec los flujos Fase E/F.

**Harness.** Extiende `tests/e2e/sprint-15c-enhance-flow.spec.ts` (E2E **REST+DB** vía `APIRequestContext` + `pg.Pool`, NO browser — coherente con su scope declarado). Corre con `pnpm ci:e2e` (= `test:e2e:build` [`nest build` + `next build`] + `playwright test`, que levanta backend+frontend+`MockEnhanceServer` vía `webServer` + requiere docker dev: postgres/redis/mailpit). Auth: helpers `loginAPI(request,email,pwd)` → tokens (sin 2FA) — patrón `superadminToken` / `clientToken` ya usado en el spec. Seeding del mock (`tests/e2e/fixtures/mock-enhance-runner.ts`): `domainSsls` (auto LE 60d por website), `websiteApps`, `websites`, subscription via POST. Label `ready-for-e2e` en el PR.

**Hallazgos de viabilidad (verificados contra código — condicionan el slicing):**
- **#6 SSO impersonation — FACTIBLE.** Admin `POST /services/:id/sso` sobre service ajeno → evento `service.admin_sso_impersonation` (plugin-utils.ts) → listener `audit-admin-sso-impersonation.listener` escribe `audit_access_log.action='admin_sso_impersonation'` con `metadata.target_user_id=service.user_id` → el cliente lo ve en el portal `GET /audit/access` (transparencia GDPR). Cadena completa testeable end-to-end.
- **#7e AdminOnly bypass — MATIZ.** El `AdminOnlyGuard` rechaza con **403 SIN audit** (cierra el request antes del controller — no hay contexto de service). La variante "403 **+ audit** `service.action_admin_only_violation`" es del **wrapper** `executeActionWithCacheInvalidation` y **YA está cubierta por el test 5** del spec (cliente invoca action `adminOnly` en su propia ruta). → G.2 añade el test guard-level (cliente → `POST /admin/services/:id/suspend` → 403, sin mutación) y documenta la distinción guard-403 vs wrapper-403+audit. NO inventar un evento nuevo en el guard (sería scope creep sin valor — el wrapper ya cubre el caso con contexto).
- **#4 CircuitBreaker recovery — BLOQUEADO por el mock.** El `MockEnhanceServer` NO expone inyección de fallo (stateless, sin endpoint `/fail`). La máquina de estados del breaker (closed→open→half-open→closed, sliding window, `CircuitOpenError`) está **100% cubierta por unit** (`circuit-breaker.spec.ts`). **Decisión frozen (recomendada):** mantener #4 como **unit-covered** + ítem de smoke manual en G.3; promover a E2E real SOLO si se añade una inyección de fallo mínima al mock (`env MOCK_ENHANCE_FAIL_MODE` o endpoint `POST /__test__/fail`) — opcional **G.2.c**, no bloqueante para cerrar §A.2 (la garantía del breaker ya está probada unit; lo que faltaría es el wiring timing real, que el smoke G.3 cubre cualitativamente). ⚠️ **Confirmar con Yasmin** al presentar este freeze.

**Slicing (PRs pequeños, 1 rama/fase):**
- **G.2.0** (rama `sprint15c-ii-fase-g2-e2e`) — este freeze + post-merge doc-sync de G.1 (`current.md`). Sin código de test.
- **G.2.a — cierre §A.2 (4/6/7e).** #6 SSO impersonation (cadena admin→evento→`audit_access`→portal) + #7e AdminOnly guard 403 (+ doc de la distinción). #4 → unit-covered + smoke G.3 (o G.2.c opcional si se aprueba inyección de fallo en el mock).
- **G.2.b — flujos lifecycle/UX (Fase E/F).** suspend (`POST /admin/services/:id/suspend`) → `status='suspended'` + evento `service.suspended` + `audit_change`(service.suspended) + `audit_access`(service_suspend_admin, target_user_id) + `ClientNote`(source_system='service', triggered_by_action='service.suspended'); banner cliente (`GET /services/:id` summary `suspension_reason`+`suspended_at`); unsuspend espejo; `ClientNote` lifecycle visible en `findByClient`; deprovision+nota (refuerza test 10); **SSL card** (seed `domainSsls` en estados valid/expiring/expired/none → `info.ssl.status`); **reconcile per-servicio** (`POST /admin/services/:id/reconcile` → evento `service.reconciled_external_change` trigger=`manual_single` + `audit_change`(service.reconciled_single)); **deep-links** (`POST /services/:id/actions/open_app_admin {appId}` admin → 200 + `data.url` + `audit_access`(service.app_admin_opened, app_id) cuando admin sobre ajeno).
- **G.2.b prerequisito DC.49**: el test de **aviso de cuota** (`POST /admin/plugins/enhance_cp/reconcile-all` → `service_quota_alerts` fila `crossed_up` + evento `service.quota_threshold_crossed`) necesita seed dinámico de `usedResources` per-subscription en el mock (DC.49, ya apuntado como housekeeping pre-G.2). Si el mock no lo soporta aún, **extender el mock primero** (mini-PR) o cubrir cuota por unit (ya cubierto en G.1 #8) + smoke G.3. ⚠️ confirmar en G.2.b.
