...
> **Audiencia**: el agente IA que continúe Fase F.3. La rama está pushada con **9 commits**; falta cerrar 1 item (B) + el close-out (CI + doc-sync + PR + merge). NO re-litigar las decisiones de abajo — ya están tomadas y materializadas. Después de F.3 viene **F.4** (SSL status + alertas de cuota — decidido con Yasmin 2026-05-12) y luego **G** (cierre Sprint 15C.II).
...
**Verificación parcial**: `tsc` backend+frontend verde, `eslint --max-warnings=0` verde ambos, suites provisioning/provisioners/core/audit/admin-plugins verdes (~420/425 en ese subconjunto + las nuevas; sin regresiones), `migrate deploy` aplicado. Pre-push hook verde en cada push.

### A.11.9.2. Pendiente de F.3 (lo que falta)

1. **B (hardening barato — acordado con Yasmin durante la revaloración 2026-05-12)**:
   - **Throttle server-side en `POST /services/:id/refresh`** (tanto el cliente como el admin). El cooldown de 10s actual es **solo frontend** (`MetricsRefreshButton`) — el endpoint es martilleable directamente; el cache TTL 60s mitiga el *coste* pero no el *abuso*. Usar `@Throttle` de `@nestjs/throttler` (verificar si ya está montado en el módulo) o un check de timestamp en Redis por `serviceId`. Aplicar a `ProvisioningController.refresh` + `AdminProvisioningController.refresh`.
   - **Añadir `plugin.reconcile_completed` al catálogo de eventos `plugin.*` de ADR-080** (hoy está documentado en ADR-083 Amendment A6 pero no listado en ADR-080 junto a los otros 5 `plugin.*`). Doc-only — un párrafo en `docs/10-decisions/adr-080-plugin-framework.md`.
2. **Close-out de F.3**:
   - `pnpm ci:check` + `pnpm ci:check:full` + boot real del backend (`node dist/src/main.js` — verificar `Nest application successfully started`, rutas `/admin|/services/:id/audit` mapeadas, sin errores DI).
   - **Doc-sync**: nuevo **ADR-083 Amendment A7** (Fase F.3 — audit timeline GAP-M + GAP-N + G4/G5/G8 + CTA reconcile; documentar las decisiones de §A.11.9.3 abajo) + actualizar este §A.11 (fila F.3 → ✅ mergeada) + (post-merge) `current.md`/`backlog.md` (patrón #61/#64/#66) + `MEMORY.md`/`project-state.md`.
   - **PR** (rama `sprint15c-ii-fase-f3-audit-timeline` → master) + si la CI de GitHub sigue bloqueada por billing (firma §A.9.10: jobs Backend/Frontend fallan en 3-11s, `log not found`), **bypass policy §6** con comentario formal en el PR (las 3 condiciones: motivo externo + `ci:check:full` verde + boot real + doc) → **squash-merge** + `--delete-branch` → post-merge doc-sync PR.
