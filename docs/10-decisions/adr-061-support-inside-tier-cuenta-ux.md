...
> **Status:** Active (refina ADR-034) · **Refinado por [ADR-075](./adr-075-support-inside-ux-lista-y-aislamiento-productos.md)** (2026-05-01) en dos puntos: materialización técnica del aislamiento del CRUD genérico de productos (filtro frontend + guard backend) y formato visual del admin (lista clicable + editor con secciones card extensibles, no formato comparador "3 cards lado a lado"). · **Refinado por [ADR-076](./adr-076-checkout-unico-support-inside-via-evento.md)** (2026-05-01) en un tercer punto: UX cliente del subscribe migra de modal in-page a redirect canónico a `/dashboard/billing/checkout` con listener `support-inside-on-service-provisioned` consumiendo evento `service.provisioned` para crear la `SupportInsideSubscription`. Garantiza un único motor de checkout para integrar Stripe/Redsys post-Sprint 14.
> **Date:** 2026-04-26
> **Domain:** support, ui
...
>
> **2026-05-01 (cierre Fase D frontend + auditoría visibilidad transversal):** Sprint 8 Fase D backend + frontend cerrados; auditoría detectó que Support Inside era "módulo aislado" en lugar de "tier de cuenta visible". 10 puntos de contacto auditados, 4 priorizados ALTA + 4 drifts canónicos (anniversary_day, services.credit_balance_eur ausente, tasks.slot_id FK pendiente, listeners support_inside.* huérfanos) movidos a sub-fase **8.D.12** (visibilidad transversal). Deudas que exceden Support Inside (DC.16 credit_balance, DC.17 tasks.slot_id, DC.18 upgrade entre planes distintos, DC.19 slots adicionales facturables, DC.20 channels alternativos, DC.21 historial de valor, DC.22 settings globales SI, DC.23 bundles, DC.24 listado admin con badge SI, DC.25 card servicio, DC.26 topbar indicador) registradas en `backlog.md`. ADR-076 formaliza el cambio modal→checkout canónico vía evento `service.provisioned`.
>
