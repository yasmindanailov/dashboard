...
### Amendment A14 (2026-06-24) — `ProvisionContext.transferAuthCode?` (EPP auth code opcional para transfer-in) (Sprint 15D.II Fase 15D.II.A)

**Contexto.** El sub-contrato de registrar (A10) modeló `ProvisionContext.operation='transfer_in'` pero no el dato que un transfer-in necesita: el **EPP auth code** (código de autorización que el registrar de origen entrega al titular para autorizar la salida del dominio). La FSM de transfer ([ADR-084 §4](./adr-084-comercio-dominios-registrar.md) + [A2](./adr-084-comercio-dominios-registrar.md#amendments)) lo exige para pasar de `awaiting_auth`/`pending` a `submitted`.

1. **`ProvisionContext.transferAuthCode?: string`** — campo **opcional**, solo significativo cuando `operation='transfer_in'`. Lo recoge el checkout (el cliente lo aporta al pedir el transfer), el orquestador lo pasa en el `ProvisionContext`, y el plugin registrar lo consume en `provision(transfer_in)` (validate + initiate). El reenvío posterior (si era inválido → `awaiting_auth`) va por la acción curada `submit_transfer_auth_code`, que reconstruye el `ProvisionContext`.
2. **R12 — es secreto.** El auth code se trata como credencial: **redactado** en logs/audit (mismo molde que `domsecret`/`get_auth_code` de [A5](./adr-081-plugin-resellerclub-specifics.md#amendments)/15D.F.1) y **nunca persistido en claro** (se usa en memoria para la llamada al registrar y se descarta).

> **Justificado por:** apertura de Sprint 15D.II (sesión 2026-06-24). **Compatibilidad:** hacia atrás — campo **opcional** additivo en `ProvisionContext` (mismo patrón que `operation` de A10 y `dnsTargetHint` de [ADR-082 A4](./adr-082-modelo-domain-hosting-dns-doctrine.md#amendments)); los plugins no-registrar y las operaciones `register`/`renew` lo ignoran. NO bumpea `contractVersion` (sigue `'v2'`).

**Heredable.** Cualquier registrar con transfer-in consume `ctx.transferAuthCode`; la recogida (checkout) y el reenvío (acción curada) son del core, capability-routed (R4). Specifics RC (endpoints `validate-transfer`/`transfer`) en [ADR-081 Amendment A7](./adr-081-plugin-resellerclub-specifics.md#amendments). Mecánica de la FSM y cobro en [ADR-084 Amendment A2](./adr-084-comercio-dominios-registrar.md#amendments).
