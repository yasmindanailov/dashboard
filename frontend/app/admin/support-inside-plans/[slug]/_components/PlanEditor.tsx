...
/* ═══════════════════════════════════════
   PlanEditor — Sprint 13 §13.AUTH Fase E (Modelo A).
   Recibe `detail` prehidratado por SC. Maneja draft/snapshot por
   seccion + saveSection que invoca updatePlanAction (revalidatePath
   recarga el SC tras guardar). ADR-075 §B.2 (5 secciones).
   ═══════════════════════════════════════ */
...
interface Props {
  initialDetail: SupportInsideAdminPlanDetail;
}

export default function PlanEditor({ initialDetail }: Props) {
  const { toast } = useToast();
  const [snapshot, setSnapshot] = useState<Snapshot>(() =>
    snapshotFromDetail(initialDetail),
  );
  const [draft, setDraft] = useState<Snapshot>(() =>
    snapshotFromDetail(initialDetail),
  );
  const [savingSection, setSavingSection] = useState<SectionKey | null>(null);
  const detailName = initialDetail.name;
  const detailSlug = initialDetail.slug;
  ...
  const dirty = useMemo<Record<SectionKey, boolean>>(
    () => ({
      ...
      support:
        ...
        snapshot.support.response_sla_hours !== draft.support.response_sla_hours ||
        !arrayEqualUnordered(
  ...
  async function saveSection(section: SectionKey) {
    ...
    if (section === 'identity') {
      ...
      if (draft.identity.short_description !== snapshot.identity.short_description) {
      ...
      if (draft.slots.slots_included !== snapshot.slots.slots_included) {
      ...
      if (
        ...
        patch.applicable_product_types = [...draft.slots.applicable_product_types];
      }
      if (draft.slots.extra_slot_price !== snapshot.slots.extra_slot_price) {
      ...
      if (
        draft.support.response_sla_hours !== snapshot.support.response_sla_hours
      ) {
    ...
    const result = await updatePlanAction(detailSlug, patch);
    if (!result.ok) {
      toast('error', result.error);
      setSavingSection(null);
      return;
    }
    const newSnap = snapshotFromDetail(result.detail);
    setSnapshot(newSnap);
    setDraft((prev) => ({ ...prev, [section]: newSnap[section] }));
  ...
  return (
    <div className={s.page}>
      ...
      <header className={s.header}>
        <h1 className={s.title}>{detailName}</h1>
        <p className={s.subtitle}>
          Slug: <code>{detailSlug}</code>. Cada sección se guarda por separado
      ...
      >
        <Input
          ...
          onChange={(e) =>
            setDraft({ ...draft, identity: { ...draft.identity, name: e.target.value } })
          }
        />
        <Input
          ...
          onChange={(e) =>
            setDraft({
              ...draft,
              identity: { ...draft.identity, short_description: e.target.value },
            })
          }
        />
        <Textarea
        ...
        <Select
          ...
          onChange={(e) =>
            setDraft({
              ...draft,
              identity: { ...draft.identity, status: e.target.value as ProductStatus },
            })
          }
          ...
          <Input
            ...
            onChange={(e) =>
              setDraft({
                ...
                pricing: {
                  ...draft.pricing,
                  monthly: { ...draft.pricing.monthly, price: e.target.value },
                },
              })
            }
          />
          ...
          <Input
            ...
            onChange={(e) =>
              setDraft({
                ...
                pricing: {
                  ...draft.pricing,
                  yearly: { ...draft.pricing.yearly, price: e.target.value },
                },
              })
            }
          />
        ...
        <div className={s.row2}>
          <Input
            ...
            onChange={(e) =>
              setDraft({
                ...draft,
                slots: { ...draft.slots, slots_included: Number(e.target.value) },
              })
            }
          />
          <Input
        ...
        <div className={s.fieldGroup}>
          ...
          <div className={s.checkboxList}>
            {APPLICABLE_PRODUCT_TYPE_DEFS.map((def) => {
              const active = draft.slots.applicable_product_types.includes(def.value);
              return (
                <button
                  key={def.value}
                  type="button"
