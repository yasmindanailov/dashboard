...
/* ═══════════════════════════════════════
   ...
   Layout: FormPage (§2.6)
   Components: Card, Input, Button, Badge,
        ...
        <div>
          ...
          <div className={styles.pricingGrid}>
            {c.selectedProduct.pricing.filter((p) => p.active).map((pricing) => (
              <button key={pricing.id} onClick={() => { c.setSelectedPricing(pricing); c.setStep('profile'); }}
                ...
                {Number(pricing.setup_fee) > 0 && (
                  <div className={styles.pricingSetup}>+ {fmt(pricing.setup_fee, pricing.currency)} setup{!c.isAdmin && <HelpTip text="Coste único de activación. Solo se cobra una vez al contratar el servicio." />}</div>
                )}
      ...
      {c.step === 'profile' && (
        <div>
          ...
          <div className={styles.profileGrid}>
            <button onClick={() => c.setSelectedProfile(null)}
            ...
            {c.profiles.map((profile) => (
              <button key={profile.id} onClick={() => c.setSelectedProfile(profile)}
                ...
                <div className={styles.profileMeta}>{profile.first_name} {profile.last_name}{profile.nif_cif && <> · {profile.nif_cif}</>}</div>
                <div className={styles.profileMeta}>{profile.address_line1}, {profile.postal_code} {profile.city}</div>
              </button>
            ))}
          </div>
